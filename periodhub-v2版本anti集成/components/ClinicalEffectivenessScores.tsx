...
const ClinicalEffectivenessScores: React.FC<
  ClinicalEffectivenessScoresProps
> = ({ language = "en" }) => {
  ...
  const titleText =
    language === "zh" ? "临床有效性评分" : "Clinical Effectiveness Scores";
      ...
      <div className="glass dark:bg-slate-800 rounded-2xl p-8 shadow-2xl">
        ...
        <div className="space-y-4">
          {scores.map((score, index) => {
            ...
            const isHighlighted =
              score.name.includes("AI-Personalized") ||
              score.name.includes("AI个性化");
        ...
        <div className="mt-6 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          <p className="text-xs text-blue-700 dark:text-blue-300">
            <strong>{language === "zh" ? "来源：" : "Source:"}</strong> ACOG
            Practice Bulletin No. 218 (2020), Dysmenorrhea and Endometriosis
          </p>
        </div>
      </div>
