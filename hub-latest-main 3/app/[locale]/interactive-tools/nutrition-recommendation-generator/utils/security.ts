...
export class InputValidator {
  ...
  // 验证选择数据
  static validateSelections(selections: any): { isValid: boolean; error?: string } {
    if (!selections || typeof selections !== 'object') {
      return { isValid: false, error: 'Invalid selections' };
    }

    // 验证月经阶段
    if (selections.menstrualPhase && typeof selections.menstrualPhase !== 'string') {
    ...
    // 验证中医体质
    if (selections.tcmConstitution && !(selections.tcmConstitution instanceof Set)) {
      return { isValid: false, error: 'Invalid TCM constitution' };
    }
...
export class RateLimiter {
  ...
  // 检查速率限制
  static checkRateLimit(ip: string): { allowed: boolean; remaining: number; resetTime: number } {
    const now = Date.now();
    ...
    requestData.count++;
    return { allowed: true, remaining: max - requestData.count, resetTime: requestData.resetTime };
  }
