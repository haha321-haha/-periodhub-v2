...
import { CycleData, CurrentCycleInfo } from '../types';
import { calculateCurrentCycle, validateCycleData } from '../utils/cycleCalculator';
...
export function useCycleTracker() {
  ...
  const updateLastPeriod = (date: string) => {
    ...
    const errors = validateCycleData({ lastPeriodDate: date, averageCycleLength: length });
    if (errors.length > 0) {
