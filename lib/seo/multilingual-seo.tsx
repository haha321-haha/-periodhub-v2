...
export function HreflangScript({ hreflangUrls }: { hreflangUrls: Record<string, string> }) {
  return (
    <>
...
export const multilingualContentConfig: MultilingualContentConfig = {
  ...
  en: {
    ...
    keywords: ["dysmenorrhea", "menstrual pain", "medical treatment", "women's health"],
    culturalContext: ["Evidence-based medicine", "Scientific research", "Clinical studies", "Professional care"]
  }
};
...
export async function generateMultilingualSEOMeta({
  ...
  // 根据语言添加文化相关的关键词
  const culturalKeywords = multilingualContentConfig[locale as 'zh' | 'en'].keywords;
  ...
  return {
    ...
    culturalContext: multilingualContentConfig[locale as 'zh' | 'en'].culturalContext
  };
}
