        ...
        <div className="flex items-center justify-between">
            ...
            {NAVIGATION.map((item) => {
              ...
              return (
                ...
                >
                  <a 
                    href={item.href}
                    className="flex items-center gap-1 text-sm font-medium text-gray-700 hover:text-purple-600 transition-colors py-2 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded"
                      ...
                      <div className="glass rounded-xl shadow-xl p-2 overflow-hidden">
                        {item.children.map((child) => (
                          <a 
                            key={child.id} 
                            href={child.href}
                            className="flex items-start gap-3 p-3 rounded-lg hover:bg-purple-50 transition-colors group/item focus:outline-none focus:ring-2 focus:ring-purple-500"
                            title={child.title}
                            onClick={() => trackEvent('menu_item_click', { item: child.title })}
                          >
                            <span className="text-xl bg-white p-2 rounded-md shadow-sm" role="img" aria-label={child.title + " icon"}>{child.icon}</span>
                            <div>
          ...
          <div className="hidden md:flex items-center gap-4">
            <button 
                ...
                className="text-sm font-medium text-purple-600 hover:bg-purple-50 px-3 py-2 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500"
          ...
          <div className="md:hidden">
            <button 
              ...
              className="p-2 text-gray-600 hover:text-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-500 rounded"
        ...
        <div className="md:hidden glass border-t border-gray-200 absolute w-full">
          <div className="px-4 pt-2 pb-6 space-y-1">
             {NAVIGATION.map((item) => {
                 ...
                 return (
                   <div key={item.label} className="py-2 border-b border-gray-100 last:border-0">
                       <a href={item.href} className="block font-medium text-gray-800 py-2">{label}</a>
                       {item.children && (
             ...
             <div className="mt-4 pt-4 space-y-3">
                <button 
                    onClick={onToggleLanguage}
                    className="w-full text-left text-sm font-medium text-purple-600 py-2"
                >
