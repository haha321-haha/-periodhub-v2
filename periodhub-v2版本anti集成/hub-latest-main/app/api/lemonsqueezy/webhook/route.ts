  ...
  try {
    ...
    const handler = (whatwgWebhooksHandler as any)({
      ...
      async onData(payload: any) {
        ...
        const email = (payload.data.attributes as { user_email?: string }).user_email || 
                     (payload.data.attributes as { customer_email?: string }).customer_email || 
                     '';
        const subscriptionId = payload.data.id;
        const status = (payload.data.attributes as { status?: string }).status;
        const variantId = (payload.data.attributes as { variant_id?: string | number }).variant_id;
        const renewsAt = (payload.data.attributes as { renews_at?: string }).renews_at;
        ...
        switch (eventName) {
          ...
          case 'order_created':
            ...
            const customerEmail = (payload.data.attributes as { user_email?: string }).user_email || 
                                 (payload.data.attributes as { customer_email?: string }).customer_email || 
                                 '';
            const orderData = {
              ...
              email: customerEmail,
              updatedAt: new Date().toISOString(),
              variantId: (payload.data.attributes as { variant_id?: string | number }).variant_id,
              plan: getPlanFromVariant((payload.data.attributes as { variant_id?: string | number }).variant_id || ''),
              type: 'one_time',
            };
