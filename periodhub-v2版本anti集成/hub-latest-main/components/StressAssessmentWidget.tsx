...
import { useWorkplaceWellnessActions, useWorkplaceWellnessStore } from "@/app/[locale]/interactive-tools/workplace-wellness/hooks/useWorkplaceWellnessStore";
...
export default function StressAssessmentWidget() {
  ...
  const { addAssessmentResult, migrateAssessmentsFromLocalStorage } = useWorkplaceWellnessActions();
  ...
  const [primaryPainPoint, setPrimaryPainPoint] = useState<"work" | "emotion" | "pain" | "default">("default");
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      import('@/lib/pro-upgrade-handler').then(() => {
  ...
  const calculatePrimaryPainPoint = (currentAnswers: number[]): "work" | "emotion" | "pain" | "default" => {
    let workScore = 0;
    let emotionScore = 0;
    let painScore = 0;
    ...
    if (normalizedWork >= normalizedEmotion && normalizedWork >= normalizedPain) return "work";
    if (normalizedEmotion >= normalizedWork && normalizedEmotion >= normalizedPain) return "emotion";
    if (normalizedPain >= normalizedWork && normalizedPain >= normalizedEmotion) return "pain";
    return "default";
  };
      ...
      setTimeout(() => {
        ...
        try {
          const assessmentRecord = {
            ...
            stressScore: score,
            stressLevel: level,
            primaryPainPoint: painPoint as "work" | "emotion" | "pain" | "default",
  ...
  const handleUnlockPremium = async (e?: React.MouseEvent<HTMLButtonElement>) => {
    // 防止重复点击
    const isProcessing = (window as { __paymentProcessing?: boolean }).__paymentProcessing || false;
    if (isProcessing) {
    ...
    try {
      ...
      const locale = hasWindow && window.location.pathname.includes('/zh') ? 'zh' : 'en';
      ...
      if (!response.ok) {
        ...
        throw new Error(`支付API错误: ${response.status} - ${errorText.substring(0, 200)}`);
      }
      
      const contentType = response.headers.get('content-type');
      if (!contentType?.includes('application/json')) {
        const textData = await response.text();
        throw new Error(`API返回了非JSON响应: ${contentType}. 响应: ${textData.substring(0, 200)}`);
      }
      ...
      const locale = typeof window !== 'undefined' && window.location.pathname.includes('/zh') ? 'zh' : 'en';
  ...
  const handleSkipPaywall = () => {
    ...
    try {
      const assessmentRecord = {
        id: `assessment_free_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        date: new Date().toISOString().split('T')[0], // YYYY-MM-DD format
  ...
  const convertAnswersToRadarData = (answers: number[]) => {
    ...
    // Work: Q1-Q3
    const workScore = answers.slice(0, 3).reduce((a, b) => a + (b || 0), 0) / 9 * 100;
    
    // Emotion: Q4-Q6
    const emotionScore = answers.slice(3, 6).reduce((a, b) => a + (b || 0), 0) / 9 * 100;
    
    // Physical: Q7-Q10
    const physicalScore = answers.slice(6, 10).reduce((a, b) => a + (b || 0), 0) / 12 * 100;
        ...
        {isPremiumUser && (
          <div className="space-y-6 mb-6">
            ...
            <div className="border-t pt-6">
              ...
              <div className="space-y-3">
                {getPersonalizedActionSteps(answers).map(
                  (step, i) => (
                    <div
                      key={i}
      ...
      <>
        ...
        <div className="fixed bottom-4 right-4 bg-black bg-opacity-90 text-white p-4 rounded-lg z-[10001] text-xs max-w-xs">
          ...
          <button
            onClick={() => {
              console.log('=== 手动触发支付 ===');
              console.log('按钮元素:', document.querySelector('[data-testid="unlock-premium"]'));
              console.log('handleUnlockPremium函数:', typeof handleUnlockPremium);
              console.log('window.handleProUpgrade:', typeof window.handleProUpgrade);
              
              // 检查按钮是否存在
              const button = document.querySelector('[data-testid="unlock-premium"]') as HTMLButtonElement;
              if (button) {
