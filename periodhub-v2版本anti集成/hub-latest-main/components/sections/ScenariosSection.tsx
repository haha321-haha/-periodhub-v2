...
interface ScenariosSectionProps {
  scenarios?: Array<{ id: string; title: string; description: string; icon?: string; badge?: string; href?: string }>;
}

export default function ScenariosSection({ scenarios: propsScenarios }: ScenariosSectionProps = {}) {
  ...
  const zones = allScenarios
    ...
    .map(s => ({
      ...
      href: s.href || `/${locale}/scenario-solutions${s.id === 'teen-zone' ? '/lifeStages' : ''}`,
    }));
  ...
  const defaultZones = zones.length === 0 ? [
    {
      id: 'teen-zone',
  ...
  const defaultCards = cards.length === 0 ? [
    {
      id: 'office',
            ...
            >
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <h3 className="text-xl font-bold dark:text-white">{zone.title}</h3>
                  {zone.badge && (
                ...
                <p 
                  ...
                  dangerouslySetInnerHTML={{ 
                    __html: markMedicalTermsInText(zone.description, localeTyped) 
                  }}
                />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {defaultCards.map((card) => (
            ...
            >
              ...
              <h4 className="font-bold text-gray-900 dark:text-white mb-1">{card.title}</h4>
              <p 
