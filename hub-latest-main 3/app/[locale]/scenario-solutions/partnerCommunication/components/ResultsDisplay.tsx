...
export default function ResultsDisplay({
  ...
  // 分享处理函数
  const handleShareClick = async (platform: 'native' | 'twitter' | 'whatsapp' | 'facebook' | 'copy') => {
    setIsSharing(true);
    setShareMessage('');

    try {
      const shareText = generateShareText(correctedTotalScore, totalQuestions, result.level, locale);
      const shareUrl = generateShareUrl(locale);
      
      const shareData: ShareData = {
        title: locale === 'zh' ? '伴侣理解度测试结果' : 'Partner Understanding Test Results',
        text: shareText,
        url: shareUrl
      };
      ...
      setShareMessage(locale === 'zh' ? '分享失败，请重试' : 'Share failed, please try again');
    } finally {
      setIsSharing(false);
    }
  };
  ...
  return (
    <div className={`max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 ${className}`}>
      ...
      {/* 分数展示 */}
      <div className="text-center mb-8">
        <div className={`text-4xl font-bold mb-4 ${getScoreColor(result.percentage)}`}>
          {result.percentage}%
        </div>
        ...
        {/* 等级徽章 */}
        <div className={`inline-block px-4 py-2 rounded-full text-sm font-medium mb-4 ${getLevelColor(result.level)}`}>
        ...
        <div className="bg-white rounded-lg p-6 shadow-md">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-600">{tCommon('correctAnswers')}:</span>
              <span className="font-medium">{correctedTotalScore}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">{tCommon('totalQuestions')}:</span>
        ...
        <div className="bg-white rounded-lg p-6 shadow-md mb-8">
          ...
          <div className="space-y-3">
            {(() => {
              ...
              // 动态检测建议数量，最多检查5个
              while (index < 5) {
                const recommendationKey = result.level + '.recommendations.' + index;
        ...
        {isSharing && (
          <div className="mt-4">
            <div className="inline-flex items-center text-gray-600">
              ...
              <span className="text-sm">{locale === 'zh' ? '分享中...' : 'Sharing...'}</span>
            </div>
          </div>
        )}
      </div>
