...
import { ChevronDown, ChevronUp, Brain, Heart, Zap, AlertTriangle } from 'lucide-react';
      ...
      <div className="relative">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          {cascadeSteps.map((step, index) => (
            <div key={step.id} className="flex items-center">
              {/* Step Card */}
              <div className={`${step.bgColor} rounded-xl p-4 shadow-md min-w-[200px]`}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm text-gray-500">{t("neuroendocrine.stepLabel")} {step.id}</span>
                  </div>
                </div>
        ...
        <div className="md:hidden flex flex-col items-center gap-2 mt-4">
          {cascadeSteps.slice(0, -1).map((step, index) => (
            <div key={`mobile-arrow-${index}`} className={`w-6 h-6 bg-gradient-to-r ${step.color} rounded-full flex items-center justify-center`}>
        ...
        <div className="mt-8 border-t pt-6">
          ...
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div className="bg-blue-50 rounded-lg p-6 border-l-4 border-blue-400">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-blue-600 font-medium">{t('neuroendocrine.acute.duration')}:</span>
                  <span className="text-blue-700">{t('neuroendocrine.acute.durationValue')}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-blue-600 font-medium">{t('neuroendocrine.acute.recovery')}:</span>
                  <span className="text-blue-700">{t('neuroendocrine.acute.recoveryValue')}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-red-50 rounded-lg p-6 border-l-4 border-red-400">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-red-600 font-medium">{t('neuroendocrine.chronic.duration')}:</span>
                  <span className="text-red-700">{t('neuroendocrine.chronic.durationValue')}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-red-600 font-medium">{t('neuroendocrine.chronic.recovery')}:</span>
                  <span className="text-red-700">{t('neuroendocrine.chronic.recoveryValue')}</span>
                </div>
              </div>
            </div>
          </div>
