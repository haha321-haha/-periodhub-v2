...
export class RecommendationEngine {
  ...
  getUrgentRecommendations(
    ...
    const contents = Array.from(this.contentDatabase.values()).filter(
      (c) => c.urgency === urgencyLevel || c.urgency === "critical",
    );
  ...
  private getCandidateContent(
    ...
    // 过滤排除的内容
    let candidates = allContent.filter(
      (c) => !options.excludeIds.includes(c.id),
    );

    // 如果指定了分类，只推荐指定分类
    if (options.forceCategories && options.forceCategories.length > 0) {
      candidates = candidates.filter((c) =>
        options.forceCategories!.includes(c.category),
      );
    }
  ...
  private contentBasedRecommendation(
    ...
    for (const content of candidates) {
      ...
      for (const interest of userInterests) {
        if (
          content.tags.some((tag) =>
            tag.toLowerCase().includes(interest.topic.toLowerCase()),
          )
        ) {
        ...
        if (
          content.title.toLowerCase().includes(interest.topic.toLowerCase())
        ) {
      ...
      if (healthProfile.severityLevel !== "unknown") {
        const severityMatch = this.matchSeverityLevel(
          content,
          healthProfile.severityLevel,
        );
    ...
    for (const content of candidates) {
      ...
      for (const { userId: similarUserId, similarity } of similarUsers) {
        const similarUserInteractions =
          this.userInteractions.get(similarUserId) || new Map();
   ...
   * 基于热门度的推荐
   */
  private popularityBasedRecommendation(
    candidates: ContentItem[],
  ): RecommendationItem[] {
    return candidates
  ...
  private healthFocusedRecommendation(
    ...
    for (const content of candidates) {
      ...
      // 紧急度匹配
      if (
        healthProfile.urgencyProfile.emergencyQueries > 0 &&
  ...
  private applyDiversityAndNovelty(
    ...
    for (const rec of recommendations) {
      ...
      // 新颖性奖励
      const noveltyBonus =
        this.calculateNoveltyBonus(rec, userProfile) * options.noveltyWeight;
  ...
  private updateContentSimilarity(newContent: ContentItem): void {
    ...
    for (const content of allContent) {
      ...
      // 更新双向相似度
      const similarities1 =
        this.contentSimilarity.get(newContent.id) || new Map();
   ...
   * 匹配治疗偏好
   */
  private matchTreatmentPreference(
    content: ContentItem,
  ...
  private calculateNoveltyBonus(
    ...
    const userInteractions =
      this.userInteractions.get(userProfile.userId) || new Map();
