...
export class LearningSystem {
  ...
  evaluateRecommendations(
    ...
    const precision =
      recommendedItems.size > 0
        ? relevantRecommended.size / recommendedItems.size
        : 0;

    const recall =
      relevantItems.size > 0
        ? relevantRecommended.size / relevantItems.size
        : 0;

    // 计算点击率
    const clickEvents = feedback.filter(
      (f) => f.feedbackType === "positive" && recommendedItems.has(f.contentId),
    );
    ...
    // 计算转化率（下载/分享等高价值行为）
    const conversionEvents = feedback.filter(
      (f) => f.feedbackValue > 0.8 && recommendedItems.has(f.contentId),
    );
    ...
    const diversityScore =
      categories.size / Math.max(recommendations.length, 1);
    ...
    const novelItems = recommendations.filter(
      (r) => !userInteractedContent.has(r.contentId),
    );
    const noveltyScore =
      recommendations.length > 0
  ...
  getPersonalizedInsights(userId: string): {
    ...
    const recommendationAccuracy =
      recentFeedback.length > 0 ? positiveCount / recentFeedback.length : 0;

    // 反馈摘要
    const totalFeedback = feedback.length;
    const positiveRate =
      feedback.filter((f) => f.feedbackValue > 0).length /
  ...
  getPerformanceHistory(): ModelPerformance[] {
    return [...this.performanceHistory].sort(
  ...
  private extractFeatures(feedback: FeedbackEvent): Map<string, number> {
    ...
    features.set(
      "position",
      1.0 / (feedback.context.recommendation_position + 1),
    );
    ...
    features.set(
      "explicit_feedback",
      feedback.context.interaction_type === "explicit" ? 1.0 : 0.0,
    );
  ...
  private evaluateABTest(testId: string): void {
    ...
    if (significanceLevel > 0.95) {
      ...
      test.winnerModel =
        aScore > bScore ? test.variantA.modelId : test.variantB.modelId;
    }
   ...
   * 提取偏好模式
   */
  private extractPreferencePattern(
  ...
  private analyzeEngagementTrend(
    ...
    const earlyAvg =
      early.reduce((sum, f) => sum + f.feedbackValue, 0) / early.length;
    const middleAvg =
      middle.reduce((sum, f) => sum + f.feedbackValue, 0) / middle.length;
    const lateAvg =
      late.reduce((sum, f) => sum + f.feedbackValue, 0) / late.length;
  ...
  private calculateStatisticalSignificance(
    ...
    const standardError = Math.sqrt(
      pooledVariance * (1 / sampleSizeA + 1 / sampleSizeB),
    );
