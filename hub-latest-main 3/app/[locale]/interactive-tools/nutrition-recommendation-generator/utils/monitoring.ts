        ...
        for (const entry of list.getEntries()) {
          if (entry.entryType === 'layout-shift') {
            ...
            if (!clsEntry.hadRecentInput) {
              this.metrics.set('CLS', (this.metrics.get('CLS') || 0) + clsEntry.value);
            }
          }
        }
      });
...
export class UserBehaviorMonitor {
  ...
  // 跟踪推荐生成
  trackRecommendationGeneration(selections: any): void {
  ...
  // 获取用户行为统计
  getUserStats(): { totalEvents: number; uniquePages: number; topEvents: Record<string, number> } {
    const uniquePages = new Set(this.events.filter(e => e.event === 'page_view').map(e => e.data?.page));
