...
export interface SEOMonitoringReport {
  ...
  metrics: {
    ...
    // 性能指标
    performance: {
      lcp: { average: number; p95: number; status: 'good' | 'needs-improvement' | 'poor' };
      fcp: { average: number; p95: number; status: 'good' | 'needs-improvement' | 'poor' };
      tbt: { average: number; p95: number; status: 'good' | 'needs-improvement' | 'poor' };
      cls: { average: number; p95: number; status: 'good' | 'needs-improvement' | 'poor' };
    };
...
export class SEOMonitor {
  ...
  async checkStructuredDataCoverage(): Promise<{
    ...
    return {
      ...
      coverage: 100,
      schemaTypes: ['MedicalWebPage', 'Article', 'SoftwareApplication', 'WebApplication', 'BreadcrumbList']
    };
  }
