...
interface CycleSetupModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { lastPeriodDate: string; averageCycleLength: number }) => void;
  initialDate?: string;
}

const CycleSetupModal: React.FC<CycleSetupModalProps> = ({ isOpen, onClose, onSave, initialDate }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm transition-opacity">
      ...
      >
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-500 p-6 text-white flex justify-between items-start">
          <div>
            <h2 id="modal-title" className="text-2xl font-bold">Setup Cycle Tracker</h2>
            <p className="text-purple-100 text-sm mt-1">Let's personalize your predictions.</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="period-date" className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
          ...
          {/* Length Input */}
          <div className="space-y-2">
            <label htmlFor="cycle-length" className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
            ...
            <div className="flex items-center gap-4">
              ...
              />
              <span className="text-xl font-bold text-purple-600 min-w-[3ch] text-center">{length}</span>
            </div>
            <p className="text-xs text-gray-500">Usually between 21 and 35 days. Default is 28.</p>
          </div>
