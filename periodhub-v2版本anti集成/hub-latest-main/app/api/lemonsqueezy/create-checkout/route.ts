...
export async function POST(req: NextRequest) {
  try {
    const { variantId, plan, painPoint, assessmentScore, email } = await req.json();
    ...
    if (!API_KEY || API_KEY.length < 50) {
      throw new Error('Invalid API Key: API Key is too short or empty. Please check your LEMONSQUEEZY_API_KEY in .env.local');
    }
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            custom_price: null,
            product_options: {
              redirect_url: `${origin}/success?checkout_id={CHECKOUT_ID}${email ? '&user_email=' + encodeURIComponent(email) : ''}`,
    ...
    if (!response.ok) {
      ...
      if (contentType?.includes('application/json') || contentType?.includes('application/vnd.api+json')) {
        ...
        if (textData.includes('<!DOCTYPE')) {
          errorData = { error: `Lemon Squeezy API returned HTML instead of JSON. Status: ${response.status}` };
        } else {
      ...
      if (response.status === 401) {
        ...
        const errorMessage = 
          (errorData && typeof errorData === 'object' && 'errors' in errorData && Array.isArray(errorData.errors) && errorData.errors[0] && typeof errorData.errors[0] === 'object' && 'detail' in errorData.errors[0])
      ...
      throw new Error(`Lemon Squeezy API error: ${response.status} - ${errorMessage}`);
    }
    ...
    if (!contentType?.includes('application/json') && !contentType?.includes('application/vnd.api+json')) {
      const textData = await response.text();
      throw new Error(`Expected JSON response but got ${contentType}. Response: ${textData.substring(0, 200)}`);
    }
    ...
    // ✅ JSON:API 响应结构检查
    if (!data.data) {
      throw new Error(`Invalid response structure from Lemon Squeezy: ${JSON.stringify(data)}`);
    }
    ...
    if (!checkoutUrl && data.data.id) {
      ...
      console.log('⚠️ Checkout URL not in initial response, fetching checkout details...', checkoutId);
      
      // 调用 GET endpoint 获取完整的 checkout 信息
      try {
        const checkoutResponse = await fetch(`https://api.lemonsqueezy.com/v1/checkouts/${checkoutId}`, {
          method: 'GET',
          headers: {
            'Accept': 'application/vnd.api+json',
            'Authorization': `Bearer ${API_KEY}`,
        ...
        if (checkoutResponse.ok) {
          ...
          if (!checkoutUrl) {
            throw new Error(`Checkout URL not found in checkout details. Response: ${JSON.stringify(checkoutData)}`);
          }
        } else {
          throw new Error(`Failed to fetch checkout details: ${checkoutResponse.status}`);
        }
      } catch (fetchError) {
        console.error('❌ Failed to fetch checkout URL:', fetchError);
        throw new Error(`Checkout created (ID: ${checkoutId}) but failed to get checkout URL: ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`);
      }
    }
    
    if (!checkoutUrl) {
      throw new Error(`Invalid response structure from Lemon Squeezy: No checkout URL found. Response: ${JSON.stringify(data)}`);
    }
