...
export async function getSubscriptionStatus(email: string): Promise<SubscriptionStatus> {
  try {
    ...
    return {
      ...
      status: subscription.status as 'active' | 'cancelled' | 'past_due' | 'none',
      subscriptionId: subscription.subscriptionId,
      type: (subscription.type || 'subscription') as 'subscription' | 'one_time',
      email: subscription.email,
      variantId: subscription.variantId,
      plan: (subscription.plan || 'unknown') as 'monthly' | 'yearly' | 'oneTime' | 'unknown',
...
export const subscriptionCache = {
  ...
  set(email: string, status: SubscriptionStatus, ttl: number = 300000) { // 默认5分钟
    try {
      const cacheData = {
        data: status,
        timestamp: Date.now(),
      ...
      localStorage.setItem(`subscription_cache_${email}`, JSON.stringify(cacheData));
    } catch (error) {
...
// 带缓存的订阅验证
export async function verifySubscriptionWithCache(email: string): Promise<SubscriptionStatus> {
  // 先检查本地缓存
