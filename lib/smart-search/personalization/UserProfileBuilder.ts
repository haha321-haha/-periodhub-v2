...
export class UserProfileBuilder {
  ...
  updateProfile(userId: string, newEvents: UserEvent[]): UserProfile {
    const existingProfile =
      this.getProfile(userId) || this.createEmptyProfile(userId);
    ...
    const updatedProfile: UserProfile = {
      ...
      healthProfile: this.mergeHealthProfile(
        existingProfile.healthProfile,
        newHealthProfile,
      ),
      behaviorProfile: this.mergeBehaviorProfile(
      ...
      interestTopics: this.mergeInterestTopics(
        existingProfile.interestTopics,
        newInterestTopics,
      ),
      sessionContext: this.buildSessionContext(
        this.behaviorTracker.getUserEvents(userId),
      ),
  ...
  private buildDemographics(events: UserEvent[]): UserDemographics {
    ...
    const devicePreference =
      (Object.entries(deviceCounts).sort(([, a], [, b]) => b - a)[0]?.[0] as
        | "mobile"
        | "tablet"
        | "desktop") || "desktop";
    ...
    const primaryLanguage =
      Object.entries(languages).sort(([, a], [, b]) => b - a)[0]?.[0] || "zh";
  ...
  private buildHealthProfile(events: UserEvent[]): HealthProfile {
    ...
    const advancedTerms = searchEvents.filter((e) =>
      /dysmenorrhea|prostaglandin|endometriosis/.test(
        e.data.query.toLowerCase(),
      ),
    ).length;
  ...
  private buildBehaviorProfile(events: UserEvent[]): BehaviorProfile {
    const patterns = this.behaviorTracker.analyzeSearchPatterns(
      events[0]?.userId || "",
    );
    const clickEvents = events.filter(
      (e) => e.type === EventType.CLICK,
    ) as ClickEvent[];
    const viewEvents = events.filter(
      (e) => e.type === EventType.VIEW,
    ) as ViewEvent[];
    ...
    // 参与度指标
    const avgTimePerContent =
      viewEvents.length > 0
        ? viewEvents.reduce((sum, e) => sum + e.data.duration, 0) /
          viewEvents.length
        : 0;

    const avgScrollDepth =
      viewEvents.length > 0
        ? viewEvents.reduce((sum, e) => sum + e.data.scrollDepth, 0) /
          viewEvents.length
        : 0;
    ...
    return {
      ...
      navigationPatterns: {
        ...
        bounceRate:
          viewEvents.filter((e) => e.data.duration < 30).length /
          Math.max(viewEvents.length, 1),
      },
    };
  }
  ...
  private buildInterestTopics(events: UserEvent[]): InterestTopic[] {
    const topics: Map<
      string,
      { frequency: number; lastInteraction: number; sources: string[] }
    > = new Map();
    ...
    for (const event of events) {
      ...
      if (event.type === EventType.SEARCH) {
        ...
        extractedTopics = this.topicExtractor.extractFromQuery(
          searchEvent.data.query,
        );
      } else if (event.type === EventType.CLICK) {
        const clickEvent = event as ClickEvent;
        if (clickEvent.data.title) {
          extractedTopics = this.topicExtractor.extractFromTitle(
            clickEvent.data.title,
          );
        }
      }
      ...
      for (const topic of extractedTopics) {
        ...
        existing.frequency++;
        existing.lastInteraction = Math.max(
  ...
  private buildSessionContext(events: UserEvent[]): SessionContext {
    ...
    const recentSessions = Array.from(sessions.entries())
      .map(([sessionId, sessionEvents]) => {
        ...
        const searchEvents = sessionEvents.filter(
          (e) => e.type === EventType.SEARCH,
        ) as SearchEvent[];
        const mainTopics = searchEvents.flatMap((e) =>
          this.topicExtractor.extractFromQuery(e.data.query),
        );
   ...
   * 合并兴趣主题
   */
  private mergeInterestTopics(
    existing: InterestTopic[],
    newTopics: InterestTopic[],
  ): InterestTopic[] {
  ...
  private calculateProfileSimilarity(
    ...
    // 健康画像相似度 (权重: 40%)
    if (
      profile1.healthProfile.severityLevel ===
      profile2.healthProfile.severityLevel
    ) {
      similarity += 0.4;
    }
    weights += 0.4;
    ...
    const behaviorSim =
      1 -
      Math.abs(
        behavior1.searchPatterns.avgQueriesPerSession -
          behavior2.searchPatterns.avgQueriesPerSession,
      ) /
        10;
  ...
  private classifyUserType(profile: UserProfile): string {
    ...
    if (
      health.treatmentPreferences.some(
        (t) => t.type === "natural" && t.confidence > 0.7,
      )
    ) {
      return "自然疗法偏好用户";
    }
  ...
  private generateRecommendations(profile: UserProfile): string[] {
    ...
    if (
      preferences.contentFormat.some((f) => f.type === "pdf" && f.score > 0.5)
    ) {
  ...
  private mergeDemographics(
    existing: UserDemographics,
    newData: UserDemographics,
  ): UserDemographics {
    return {
      ...existing,
      ...
      activityPeakHours: [
        ...new Set([
          ...existing.activityPeakHours,
          ...newData.activityPeakHours,
        ]),
      ].slice(0, 5),
    };
  }
   ...
   * 合并健康画像
   */
  private mergeHealthProfile(
    existing: HealthProfile,
    newData: HealthProfile,
  ): HealthProfile {
    return {
      severityLevel:
        newData.severityLevel !== "unknown"
          ? newData.severityLevel
          : existing.severityLevel,
      symptomPatterns: [
        ...new Set([...existing.symptomPatterns, ...newData.symptomPatterns]),
      ],
      treatmentPreferences: this.mergeTreatmentPreferences(
        existing.treatmentPreferences,
        newData.treatmentPreferences,
      ),
      knowledgeLevel:
        newData.knowledgeLevel !== "beginner"
          ? newData.knowledgeLevel
          : existing.knowledgeLevel,
      urgencyProfile: {
        emergencyQueries:
          existing.urgencyProfile.emergencyQueries +
          newData.urgencyProfile.emergencyQueries,
        immediateNeedQueries:
          existing.urgencyProfile.immediateNeedQueries +
          newData.urgencyProfile.immediateNeedQueries,
        planningQueries:
          existing.urgencyProfile.planningQueries +
          newData.urgencyProfile.planningQueries,
      },
    };
  }
  ...
  private mergeTreatmentPreferences(
    ...
    for (const pref of newData) {
      ...
      merged.set(
        pref.type,
        Math.min(existingConf + pref.confidence * 0.3, 1.0),
      );
    }
   ...
   * 合并行为画像
   */
  private mergeBehaviorProfile(
    existing: BehaviorProfile,
    newData: BehaviorProfile,
  ): BehaviorProfile {
    return {
      searchPatterns: {
        avgQueriesPerSession:
          (existing.searchPatterns.avgQueriesPerSession +
            newData.searchPatterns.avgQueriesPerSession) /
          2,
        avgQueryLength:
          (existing.searchPatterns.avgQueryLength +
            newData.searchPatterns.avgQueryLength) /
          2,
        queryRefinementRate:
          (existing.searchPatterns.queryRefinementRate +
            newData.searchPatterns.queryRefinementRate) /
          2,
        searchDepth: newData.searchPatterns.searchDepth,
      },
      engagementMetrics: {
        avgTimePerContent:
          (existing.engagementMetrics.avgTimePerContent +
            newData.engagementMetrics.avgTimePerContent) /
          2,
        avgScrollDepth:
          (existing.engagementMetrics.avgScrollDepth +
            newData.engagementMetrics.avgScrollDepth) /
          2,
        returnVisitRate:
          (existing.engagementMetrics.returnVisitRate +
            newData.engagementMetrics.returnVisitRate) /
          2,
        contentCompletionRate:
          (existing.engagementMetrics.contentCompletionRate +
            newData.engagementMetrics.contentCompletionRate) /
          2,
      },
  ...
  private mergePreferences(
    ...
    return {
      ...existing,
      contentFormat:
        newData.contentFormat.length > 0
          ? newData.contentFormat
