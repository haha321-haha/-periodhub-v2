...
export async function generateMetadata({
  ...
  return {
    ...
    keywords: locale === 'zh' 
      ...
      : ['period pain', 'menstrual cramps', 'dysmenorrhea', 'pain management', 'women health', 'medical guide'],
        ...
        <main className="max-w-4xl mx-auto space-y-16 md:space-y-24">
          ...
          <section className="bg-white p-6 md:p-10 rounded-2xl shadow-lg">
            ...
            <div className="grid md:grid-cols-3 gap-6 text-center">
              <div className="p-4">
                <div className="flex items-center justify-center w-12 h-12 bg-primary-100 text-primary-600 font-bold text-xl rounded-full mx-auto mb-4">1</div>
                <h3 className="font-bold text-lg text-primary-700 mb-2">{t('explanation.step1.title')}</h3>
                <p className="text-sm text-neutral-600">{t('explanation.step1.description')}</p>
              </div>
              <div className="p-4">
                <div className="flex items-center justify-center w-12 h-12 bg-primary-100 text-primary-600 font-bold text-xl rounded-full mx-auto mb-4">2</div>
                <h3 className="font-bold text-lg text-primary-700 mb-2">{t('explanation.step2.title')}</h3>
                <p className="text-sm text-neutral-600">{t('explanation.step2.description')}</p>
              </div>
              <div className="p-4">
                <div className="flex items-center justify-center w-12 h-12 bg-primary-100 text-primary-600 font-bold text-xl rounded-full mx-auto mb-4">3</div>
                <h3 className="font-bold text-lg text-primary-700 mb-2">{t('explanation.step3.title')}</h3>
                <p className="text-sm text-neutral-600">{t('explanation.step3.description')}</p>
              </div>
            </div>
            ...
            <div className="mt-8 bg-primary-50 border border-primary-200 text-primary-800 p-4 rounded-lg text-center flex items-center justify-center gap-3">
              ...
              <p>
                <strong>{t('explanation.analogy.title')}</strong> {t('explanation.analogy.description')}
              </p>
            </div>
          </section>
            ...
            <div className="space-y-12">
              ...
              <div className="bg-white p-6 md:p-8 rounded-2xl shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-neutral-600 mb-3">{t('toolkit.immediate.heat.description')}</p>
                  <ul className="list-disc list-inside space-y-2 text-neutral-600">
                    {t.raw('toolkit.immediate.heat.items').map((item: string, index: number) => (
                ...
                <div className="space-y-6">
                  {t.raw('toolkit.medication.options').map((med: any, index: number) => (
                    <div key={index}>
                      ...
                      <p className="text-sm text-neutral-500">{med.bestFor}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {t.raw('stories.cases').map((story: any, index: number) => (
                <div key={index} className="bg-white p-6 rounded-xl shadow-lg text-left">
                  ...
                  <p className="text-neutral-600 mb-3">
                    <strong>{story.problemTitle}</strong> {story.problemDescription}
                  </p>
                  <p className="text-neutral-600">
                    <strong>{story.solutionTitle}</strong> {story.solutionDescription}
                  </p>
                </div>
              ))}
            </div>
