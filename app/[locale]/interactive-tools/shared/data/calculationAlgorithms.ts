...
function generateSymptomRecommendations(
    ...
    en: {
      ...
      duration: {
        long: {
          ...
          longTerm: ["Track pain duration patterns and share with doctor for long-term management plan"],
        },
        variable: {
          longTerm: ["Keep detailed log of pain triggers and relief methods to identify patterns"],
        },
      },
      symptoms: {
        nausea: ["Avoid greasy foods, eat small frequent meals, consider ginger or peppermint tea"],
        ...
        dizziness: ["Avoid sudden standing, maintain stable blood sugar, stay hydrated"],
      },
      location: {
        lower_abdomen: ["Focus heat on lower abdomen, try child's pose yoga position"],
  ...
  if (recommendationTexts[localeKey][painLevelKey]) {
    ...
    (recommendations.longTerm as any[]).push(
      ...(recommendationTexts[localeKey][painLevelKey].longTerm as any),
    );
  }

  // 基于持续时间添加建议
  if (painDuration === 'long' && recommendationTexts[localeKey].duration.long) {
    (recommendations.immediate as any[]).push(
      ...(recommendationTexts[localeKey].duration.long.immediate as any),
    );
    (recommendations.longTerm as any[]).push(
      ...(recommendationTexts[localeKey].duration.long.longTerm as any),
    );
  } else if (painDuration === 'variable' && recommendationTexts[localeKey].duration.variable) {
    (recommendations.longTerm as any[]).push(
      ...(recommendationTexts[localeKey].duration.variable.longTerm as any),
    );
  }

  // 基于伴随症状添加针对性建议
  if (Array.isArray(accompanyingSymptoms) && accompanyingSymptoms.length > 0) {
    accompanyingSymptoms.forEach((symptom: string) => {
      const symptomKey = symptom as keyof typeof recommendationTexts.zh.symptoms;
      if (recommendationTexts[localeKey].symptoms[symptomKey]) {
        (recommendations.immediate as any[]).push(
          ...(recommendationTexts[localeKey].symptoms[symptomKey] as any),
        );
      }
    });
  }
  ...
  if (Array.isArray(painLocation) && painLocation.length > 0) {
    painLocation.forEach((location: string) => {
      const locationKey = location as keyof typeof recommendationTexts.zh.location;
      if (recommendationTexts[localeKey].location[locationKey]) {
        (recommendations.immediate as any[]).push(
          ...(recommendationTexts[localeKey].location[locationKey] as any),
        );
      }
    });
  }
  ...
  if (reliefPreference === "natural") {
    (recommendations.immediate as any[]).unshift(
      ...(recommendationTexts[localeKey].natural.immediate as any),
    );
    (recommendations.longTerm as any[]).push(
      ...(recommendationTexts[localeKey].natural.longTerm as any),
    );
  }
  if (reliefPreference === "medical") {
    (recommendations.longTerm as any[]).unshift(
...
export function calculateDetailedImpact(
  ...
  return generateSymptomRecommendations(
    { painLevel, painDuration, reliefPreference, accompanyingSymptoms, painLocation },
    locale,
    isSevere,
    summary,
    score
...
export function calculateMedicalImpact(
  ...
  const combinedRecommendations = {
    immediate: [
      ...(symptomResult.recommendations?.immediate || []),
    ],
    longTerm: [
  ...
  return {
    ...
    summary: [
      ...
      `${locale === "zh" ? "职场影响评分" : "Workplace Impact Score"}: ${workplaceResult.score}/100`,
    ],
...
export function calculateWorkplaceImpact(
  ...
  if (score > 75) {
    profile = (
      suggestionTexts[locale as keyof typeof suggestionTexts].supportive as any
    ).profile;
    suggestions.push(
      ...(
        suggestionTexts[locale as keyof typeof suggestionTexts]
          .supportive as any
      ).suggestions,
    );
  } else if (score > 40) {
    profile = (
      suggestionTexts[locale as keyof typeof suggestionTexts].adaptive as any
    ).profile;
    suggestions.push(
      ...(
        suggestionTexts[locale as keyof typeof suggestionTexts].adaptive as any
    ...
    profile = (
      suggestionTexts[locale as keyof typeof suggestionTexts].challenging as any
    ).profile;
    suggestions.push(
      ...(
        suggestionTexts[locale as keyof typeof suggestionTexts]
