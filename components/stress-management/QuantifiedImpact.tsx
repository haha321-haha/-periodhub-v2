...
export default function QuantifiedImpact() {
  ...
  const menstrualImpact = [
    ...
    },
    {
      ...
      highStress: t('quantifiedImpact.menstrualImpact.painIntensity.highStress'),
      icon: AlertTriangle
    },
    {
  ...
  const ProgressBar = ({ value, max, color, showValue = true }: {
    value: number;
    max: number;
    color: string;
    showValue?: boolean;
  ...
  const getIntensityLevel = (increase: number) => {
    if (increase >= 70) return { level: 'high', emoji: '🔴🔴🔴', color: 'text-red-600' };
    if (increase >= 50) return { level: 'medium', emoji: '🔴🔴', color: 'text-orange-600' };
        ...
        <div className="space-y-6">
          {impactMetrics.map((metric) => {
            ...
            return (
              <div
                key={metric.id}
                className={`${metric.bgColor} rounded-lg p-6 cursor-pointer transition-all hover:shadow-md ${
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 bg-gradient-to-r ${metric.color} rounded-full flex items-center justify-center`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-current border-opacity-20">
                    <div className="bg-white rounded-lg p-4">
                      ...
                      <p className="text-gray-600 text-sm">
                        {t(`quantifiedImpact.emotionalImpact.details.${metric.id}`)}
                      </p>
                    </div>
                  </div>
                )}
