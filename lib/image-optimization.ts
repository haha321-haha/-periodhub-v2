...
// 根据图片用途获取优化配置
export function getImageConfig(type: 'hero' | 'content' | 'thumbnail' | 'decorative'): ImageOptimizationConfig {
...
export function getOptimizedImagePath(
  ...
  if (size) {
    return `${pathParts.join('/')}/optimized/${format}/${nameWithoutExt}-${size}w.${format}`;
  }
  
  return `${pathParts.join('/')}/optimized/${format}/${nameWithoutExt}.${format}`;
}
...
export function trackImageLoadPerformance(imageSrc: string, loadTime: number) {
  if (typeof window !== 'undefined' && (window as any).gtag) {
    (window as any).gtag('event', 'image_load_performance', {
      image_src: imageSrc,
      load_time: loadTime,
      performance_rating: loadTime < 1000 ? 'fast' : loadTime < 3000 ? 'medium' : 'slow'
    });
  }
}
...
// 图片优化建议
export function getImageOptimizationTips(currentSize: number, recommendedSize: number) {
  const tips = [];
