...
import { analyzeFeedback, FeedbackAnalysis } from './feedbackAnalyzer';
...
export function generateOptimizationSuggestions(
  ...
  if (feedbackAnalysis.averageRating < 3 && feedbackAnalysis.averageRating > 0) {
    suggestions.push({
      priority: 'high',
      category: 'content',
      title: '提升推荐内容质量',
...
export function generateOptimizationReport(
  ...
  const report = generateOptimizationSuggestions(result, userData, feedbackHistory);
  const qualityAnalysis = analyzeRecommendationQuality(result, userData);
  ...
  if (report.suggestions.length > 0) {
    ...
    report.suggestions.forEach((suggestion, index) => {
      reportText += `  ${index + 1}. [${suggestion.priority.toUpperCase()}] ${suggestion.title}\n`;
