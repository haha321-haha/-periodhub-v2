...
export default function P2AdvancedFeatures({ locale, userId }: P2AdvancedFeaturesProps) {
  const t = useTranslations('interactiveTools');
  const [activeFeature, setActiveFeature] = useState<'analytics' | 'ai' | 'social' | 'sync' | 'reports'>('analytics');
  ...
  const features = [
    {
      ...
      description: locale === 'zh' ? '深入了解您的健康模式和趋势' : 'Deep insights into your health patterns and trends',
      icon: BarChart3,
      color: 'blue',
    ...
    {
      id: 'ai',
      title: locale === 'zh' ? 'AI个性化推荐' : 'AI Personalized Recommendations',
      description: locale === 'zh' ? '基于机器学习的智能建议系统' : 'Machine learning-powered intelligent recommendation system',
      icon: Brain,
      color: 'purple',
    ...
    },
    {
      ...
      description: locale === 'zh' ? '与社区成员分享经验获得支持' : 'Share experiences with community members and get support',
      icon: Users,
      color: 'green',
    ...
    },
    {
      ...
      description: locale === 'zh' ? '跨设备安全同步您的健康数据' : 'Securely sync your health data across devices',
      icon: Cloud,
      color: 'indigo',
    ...
    },
    {
      ...
      description: locale === 'zh' ? '生成专业的个人健康报告' : 'Generate professional personal health reports',
      icon: FileText,
      color: 'emerald',
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          {features.map((feature) => {
            ...
            return (
              ...
              >
                ...
                <h4 className={`font-semibold text-sm mb-2 ${isActive ? 'text-white' : 'text-gray-900'}`}>
                  {feature.title}
                </h4>
                <div className="space-y-1">
                  {Object.entries(feature.stats).map(([key, value]) => (
                    <div key={key} className={`text-xs ${isActive ? 'text-white/80' : 'text-gray-600'}`}>
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600">
              {locale === 'zh' ? '基于50+医学研究，92%准确率' : 'Based on 50+ medical studies, 92% accuracy rate'}
            </p>
          </div>
        </div>
      </div>
