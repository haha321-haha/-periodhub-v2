...
export default function PricingPageClient() {
  ...
  const plans = [
    ...
    },
    {
      ...
      originalPrice: null,
      savings: null,
      features: Array.isArray(t.raw('plans.oneTime.features')) ? (t.raw('plans.oneTime.features') as string[]) : [],
      recommended: false
    }
  ];
  ...
  const handleUpgrade = (planId: string) => {
    if (window.handleProUpgrade) {
      ...
      console.error('全局升级函数未找到');
      alert('升级功能暂时不可用，请稍后重试');
    }
  };

  const faqItems = t.raw('faq.items');
  const safeFaqItems = Array.isArray(faqItems) ? (faqItems as Array<{ question: string; answer: string }>) : [];
        ...
        <div className="max-w-6xl mx-auto px-4 text-center">
          <div className="flex justify-center items-center gap-2 mb-4">
            ...
            <h1 className="text-4xl font-bold" suppressHydrationWarning>
              {t('hero.title').replace(/PeriodHub ProPeriodHub Pro/g, 'PeriodHub Pro').replace(/(PeriodHub Pro){2,}/g, 'PeriodHub Pro')}
            </h1>
          </div>
          ...
          <div className="flex justify-center gap-4">
            <div className="text-center">
              ...
              <div className="text-purple-200">{t('hero.stats.assessments')}</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold">98%</div>
              <div className="text-purple-200">{t('hero.stats.satisfaction')}</div>
            </div>
            <div className="text-center">
      ...
      <div className="max-w-6xl mx-auto px-4 py-16">
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`relative rounded-2xl shadow-lg p-8 transition-all ${plan.recommended
            ...
            >
                ...
                <div className="mb-4">
                  ...
                  {plan.savings && (
                    <div className="text-sm text-green-600 font-semibold">
                      {locale === 'zh' ? `节省 ${plan.savings}` : `Save ${plan.savings}`}
                    </div>
                  )}
                </div>
              </div>
              ...
              <Button
                ...
                className={`w-full py-3 ${plan.recommended
                    ? 'bg-gradient-to-r from-purple-600 to-pink-500 hover:shadow-lg'
                    : 'bg-gray-900 hover:bg-gray-800'
                  }`}
              >
                {plan.id === 'oneTime' ? t('buttons.buyNow') : t('buttons.subscribe')}
              </Button>
            </div>
          ))}
        </div>
