  ...
  persist(
    (set, get) => ({
      ...
      duplicateTemplate: (id) => {
        ...
        const template = state.exportTemplates.find(t => t.id === id);
        if (template) {
          const newTemplate: Omit<ExportTemplate, 'id' | 'createdAt' | 'updatedAt'> = {
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        if (state) {
          // 确保 Date 对象正确反序列化
          if (state.calendar.currentDate && typeof state.calendar.currentDate === 'string') {
            state.calendar.currentDate = new Date(state.calendar.currentDate);
          }
          if (state.calendar.selectedDate && typeof state.calendar.selectedDate === 'string') {
...
export const useUserPreferencesActions = () => {
  const updateUserPreferences = useWorkplaceWellnessStore((state) => state.updateUserPreferences);
  ...
  const toggleAnimations = useWorkplaceWellnessStore((state) => state.toggleAnimations);
  const toggleCompactMode = useWorkplaceWellnessStore((state) => state.toggleCompactMode);
  const updateNotificationSettings = useWorkplaceWellnessStore((state) => state.updateNotificationSettings);
  const updatePrivacySettings = useWorkplaceWellnessStore((state) => state.updatePrivacySettings);
  const updateAccessibilitySettings = useWorkplaceWellnessStore((state) => state.updateAccessibilitySettings);
  const validateSettings = useWorkplaceWellnessStore((state) => state.validateSettings);
  const resetPreferences = useWorkplaceWellnessStore((state) => state.resetPreferences);
...
export const useExportTemplateActions = () => {
  const addExportTemplate = useWorkplaceWellnessStore((state) => state.addExportTemplate);
  const updateExportTemplate = useWorkplaceWellnessStore((state) => state.updateExportTemplate);
  const deleteExportTemplate = useWorkplaceWellnessStore((state) => state.deleteExportTemplate);
  const setActiveTemplate = useWorkplaceWellnessStore((state) => state.setActiveTemplate);
  const loadTemplate = useWorkplaceWellnessStore((state) => state.loadTemplate);
  const duplicateTemplate = useWorkplaceWellnessStore((state) => state.duplicateTemplate);
...
export const useBatchExportActions = () => {
  const createBatchExport = useWorkplaceWellnessStore((state) => state.createBatchExport);
  const updateBatchItemStatus = useWorkplaceWellnessStore((state) => state.updateBatchItemStatus);
  const cancelBatchExport = useWorkplaceWellnessStore((state) => state.cancelBatchExport);
  const retryFailedItems = useWorkplaceWellnessStore((state) => state.retryFailedItems);
  const clearBatchExport = useWorkplaceWellnessStore((state) => state.clearBatchExport);
...
export const useExportHistoryActions = () => {
  const addExportHistory = useWorkplaceWellnessStore((state) => state.addExportHistory);
  const clearExportHistory = useWorkplaceWellnessStore((state) => state.clearExportHistory);
  const deleteExportHistory = useWorkplaceWellnessStore((state) => state.deleteExportHistory);
...
export const useSystemSettingsActions = () => {
  const updateSystemSettings = useWorkplaceWellnessStore((state) => state.updateSystemSettings);
  const resetSystemSettings = useWorkplaceWellnessStore((state) => state.resetSystemSettings);
