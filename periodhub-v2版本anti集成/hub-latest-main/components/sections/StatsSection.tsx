...
export default function StatsSection({ stats: propsStats }: StatsSectionProps = {}) {
  const t = useTranslations('v2Home');

  // Use props stats if provided, otherwise get from translations
  const stats = propsStats || [
    { value: t('stats.active_users.value'), label: t('stats.active_users.label') },
    { value: t('stats.user_rating.value'), label: t('stats.user_rating.label') },
    { value: t('stats.medical_guides.value'), label: t('stats.medical_guides.label') },
    { value: t('stats.hipaa_compliant.value'), label: t('stats.hipaa_compliant.label') },
  ];
