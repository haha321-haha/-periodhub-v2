 ...
 * 解析日志文件生成趋势数据
 */
function parseValidationLog(logPath = 'logs/schema-validation.log') {
 ...
 */
function generateRichResultsReport(trendData) {
  const passRate = trendData.totalPages > 0 ? (trendData.passedPages / trendData.totalPages * 100).toFixed(1) : '0';
  const statusColor = trendData.averageScore >= 95 ? 'green' : trendData.averageScore >= 80 ? 'orange' : 'red';
    ...
    <section id="rich-results-trend" class="report-section">
      ...
      <div class="trend-summary">
        <div class="summary-cards">
          ...
          <div class="summary-card">
            <h3>质量趋势</h3>
            <div class="trend-indicator ${getTrendIndicator(trendData.dailyTrends)}">
 ...
 * 生成趋势图表
 */
function generateTrendChart(dailyTrends) {
  ...
  // 生成 SVG 路径
  const scorePath = dailyTrends.map((trend, index) => {
    const x = dailyTrends.length === 1 
      ? chartWidth / 2 
      : padding + (index * (chartWidth - 2 * padding) / (dailyTrends.length - 1));
    const y = padding + (chartHeight - 2 * padding) * (1 - trend.averageScore / maxScore);
  ...
  return `
    <div class="chart-container">
      ...
      <svg width="${chartWidth}" height="${chartHeight}" viewBox="0 0 ${chartWidth} ${chartHeight}">
        <!-- 网格线 -->
        <line x1="${padding}" y1="${padding}" x2="${padding}" y2="${chartHeight - padding}" stroke="#e1e5e9" stroke-width="1"/>
        <line x1="${padding}" y1="${chartHeight - padding}" x2="${chartWidth - padding}" y2="${chartHeight - padding}" stroke="#e1e5e9" stroke-width="1"/>
        ...
        ${dailyTrends.map((trend, index) => {
          const x = dailyTrends.length === 1 
            ...
            : padding + (index * (chartWidth - 2 * padding) / (dailyTrends.length - 1));
          return `
            <text x="${x}" y="${chartHeight - 10}" text-anchor="middle" font-size="12" fill="#666">
              ${trend.date.slice(5)}
            </text>
          `;
        }).join('')}
 ...
 * 生成常见问题表格
 */
function generateCommonIssuesTable(commonIssues) {
  if (commonIssues.length === 0) {
    return '<div class="chart-container"><p>🎉 太棒了！暂未发现重复性问题</p></div>';
  }
  
  const rows = commonIssues.map(issue => `
    <tr>
      <td>${issue.issue}</td>
      <td>${issue.count}</td>
      <td>${issue.affectedPages.length}</td>
      <td>${issue.affectedPages.slice(0, 3).join(', ')}${issue.affectedPages.length > 3 ? '...' : ''}</td>
    </tr>
  `).join('');
 ...
 * 生成改进建议
 */
function generateImprovementSuggestions(trendData) {
  const suggestions = [];
  ...
  if (trendData.commonIssues.length > 0) {
    suggestions.push(`关注重复出现的问题："${trendData.commonIssues[0].issue}"，可能需要系统性修复`);
  }
  ...
  if (trendData.dailyTrends.length > 1) {
    ...
    if (lastTwoDays[1].averageScore < lastTwoDays[0].averageScore) {
      suggestions.push('近期验证分数呈下降趋势，建议检查最近的代码变更是否引入了新问题');
    } else {
 ...
 * 获取趋势指示器
 */
function getTrendIndicator(dailyTrends) {
  if (dailyTrends.length < 2) return 'trend-stable';
  
  const recent = dailyTrends.slice(-3);
  const averageScore = recent.reduce((sum, d) => sum + d.averageScore, 0) / recent.length;
  const older = dailyTrends.slice(0, -3);
  
  if (older.length === 0) return 'trend-stable';
  
  const olderScore = older.reduce((sum, d) => sum + d.averageScore, 0) / older.length;
 ...
 * 获取趋势图标
 */
function getTrendIcon(dailyTrends) {
 ...
 * 获取趋势描述
 */
function getTrendDescription(dailyTrends) {
  const indicator = getTrendIndicator(dailyTrends);
  
  switch (indicator) {
    case 'trend-up': return '质量持续提升';
    case 'trend-down': return '质量有所下降';
    default: return '质量保持稳定';
  }
}
 ...
 * 更新监控报告
 */
function updateMonitoringReport(reportPath = 'reports/AEO-Monitoring-Report.html') {
  try {
    // 解析趋势数据
    ...
    if (fs.existsSync(reportPath)) {
      ...
      if (startIndex !== -1 && endIndex !== -1) {
        ...
        if (bodyEndIndex !== -1) {
          ...
          fs.writeFileSync(reportPath, newContent);
          console.log('✅ Rich Results 趋势分析已添加到 AEO 监控报告');
        } else {
          // 如果找不到 </body> 标签，追加到文件末尾
          const newContent = content + richResultsReport;
          fs.writeFileSync(reportPath, newContent);
          console.log('✅ Rich Results 趋势分析已追加到报告末尾');
        }
      }
    } else {
      // 创建新报告
      ...
      console.log('✅ AEO 监控报告已创建');
    }
    
    return true;
  } catch (error) {
    console.error('❌ 更新监控报告失败:', error);
    return false;
  }
}
