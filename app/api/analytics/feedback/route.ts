...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingIndex >= 0) {
      feedbackStore[existingIndex] = { ...feedbackStore[existingIndex], ...cleanedFeedback };
    } else {
    ...
    console.log(`📝 收到用户反馈: ${feedback.userId}, 评分: ${feedback.rating}, 情感: ${feedback.sentiment}`);
...
function cleanFeedbackData(feedback: RealFeedbackData): RealFeedbackData {
  ...
  // 主题验证
  const validTopics = ['performance', 'ui_ux', 'functionality', 'content', 'mobile'];
  cleaned.topics = cleaned.topics.filter(topic => validTopics.includes(topic));
