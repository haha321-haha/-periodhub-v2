...
import { getAllCTATrackingData, exportCTADataToCSV } from '@/hooks/useCTATracking';
import { analyzeABTest, getRecommendedSampleSize } from '@/config/ab-tests.config';
...
export default function ABTestDashboard() {
  ...
  const loadMetrics = () => {
    ...
    try {
      ...
      // 分析事件数据
      const metrics = analyzeEvents(trackingData.events);
      setMetrics(metrics);
  ...
  const analyzeEvents = (events: any[]): ABTestMetrics[] => {
      ...
      if (event.eventName === 'hero_cta_impression') {
        ...
        if (!metricsMap.has(key)) {
          metricsMap.set(key, {
            ...
            variantId: event.ab_test_variant || 'unknown',
            impressions: 0,
            clicks: 0,
            ctr: 0
          });
    ...
    return (
      <div className="flex items-center justify-center p-8">
        <div className="animate-pulse text-center">
          <div className="text-lg font-medium mb-2">Loading A/B Test Data...</div>
          <div className="text-gray-500">Gathering analytics from local storage</div>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Real-time analytics for your optimization tests</p>
        </div>
      ...
      {Object.keys(userVariants).length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Current A/B test variants assigned to you</CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-4 h-4 rounded-full ${getVariantColor(result.variant === 'Control' ? 'control' : 'optimized')}`} />
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">{result.impressions.toLocaleString()}</div>
                  <div className="text-sm text-gray-500">Impressions</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">{result.clicks.toLocaleString()}</div>
                  <div className="text-sm text-gray-500">Clicks</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600">{result.ctr.toFixed(2)}%</div>
                  <div className="text-sm text-gray-500">CTR</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-orange-600">{result.confidence}%</div>
                  ...
                  <tr key={index} className="border-b">
                    ...
                    <td className="p-2">
                      <div className="flex items-center gap-2">
                        <div className={`w-3 h-3 rounded-full ${getVariantColor(metric.variantId)}`} />
                        {metric.variantId}
                      </div>
                    </td>
                    <td className="text-right p-2">{metric.impressions.toLocaleString()}</td>
                    <td className="text-right p-2">{metric.clicks.toLocaleString()}</td>
                    <td className="text-right p-2 font-medium">{metric.ctr.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export function ABTestSummary({ testId }: { testId: string }) {
  ...
  return (
    <div className="flex items-center gap-4 text-sm">
      {summary.map((variant: any, index: number) => (
