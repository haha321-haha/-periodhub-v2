      ...
      if (error.name === 'QuotaExceededError' || error.code === 22) {
        ...
        try {
          ...
          try {
            ...
            console.warn('Failed to save to localStorage after cleanup, falling back to sessionStorage');
            sessionStorage.setItem(key, minimalDataString);
            console.log('Saved minimal data to sessionStorage as fallback');
          }
        } catch (cleanupError) {
...
// 工具函数：计算评估结果
const calculateResult = (answers: AssessmentAnswer[], questions: Question[], locale: string = 'en'): AssessmentResult | null => {
    ...
    if (question.type === 'scale') {
      ...
      if (question.options) {
        const maxOptionWeight = Math.max(
          ...question.options.map(opt => typeof opt.weight === 'number' ? opt.weight : 0)
        );
  ...
  // 生成建议（传入 locale）
  const recommendations = generateRecommendations(severity, answers, questions, locale);
...
const recommendationTexts: {
  en: Record<string, { title: string; description: string; actionSteps: string[] }>;
  zh: Record<string, { title: string; description: string; actionSteps: string[] }>;
} = {
  en: {
    immediate_medical_attention: {
      title: 'Seek Immediate Medical Attention',
      description: 'Your symptoms require immediate medical evaluation to rule out serious conditions.',
      actionSteps: [
    ...
    medical_evaluation: {
      ...
      description: 'A thorough medical evaluation is recommended for proper diagnosis and treatment.',
      actionSteps: [
    ...
    stress_management: {
      ...
      description: 'High stress levels can worsen menstrual symptoms. Consider stress-reduction techniques.',
      actionSteps: [
    ...
    lifestyle_changes: {
      ...
      description: 'Regular exercise and a balanced diet can help reduce menstrual pain.',
      actionSteps: [
    ...
    self_care: {
      ...
      description: 'Simple self-care measures can significantly reduce mild menstrual pain.',
      actionSteps: [
    ...
    medical_consultation: {
      ...
      description: 'Consider consulting with a healthcare provider for personalized treatment options.',
      actionSteps: [
    ...
    pain_management: {
      ...
      description: 'Effective pain management strategies for moderate symptoms.',
      actionSteps: [
...
const generateRecommendations = (
  ...
  if (stressAnswer && typeof stressAnswer.value === 'number' && stressAnswer.value > 7) {
    recommendations.push({
  ...
  const exerciseAnswer = answers.find(a => a.questionId === 'exercise_frequency');
  if (exerciseAnswer && (exerciseAnswer.value === 'rarely' || exerciseAnswer.value === 'monthly')) {
...
export const usePainImpactCalculator = (userId?: string): PainImpactCalculatorHookReturn => {
  ...
  const storageKey = createStorageKey(userId || 'anonymous', 'pain_impact_calculator');
  ...
  const startAssessment = useCallback((locale: string) => {
    const sessionId = `pain_impact_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
