...
export const useSmartPreload = () => {
  useEffect(() => {
    ...
    const shouldPreloadWebpack = () => {
      ...
      const hasLocalStorageFlag = localStorage.getItem('enableWebpack') === 'true';
      const hasSessionStorageFlag = sessionStorage.getItem('enableWebpack') === 'true';
  ...
  // 返回预加载状态（可选）
  return {
    isWebpackPreloaded: typeof window !== 'undefined' && 
...
export const checkWebpackUsage = () => {
  ...
  // 检查webpack是否在全局对象中
  const hasWebpack = typeof (window as any).webpackChunkName !== 'undefined' ||
