          ...
          sections: [
            {
              title: "呼吸缓解法",
              content:
                "深呼吸技巧可以有效缓解经期疼痛，通过调节自律神经系统来减轻痉挛。",
              pageNumber: 3,
            },
            {
              title: "热疗应用",
              content:
                "局部热敷是最常用且有效的疼痛缓解方法之一，温度控制在40-45度最佳。",
              pageNumber: 5,
            },
          ],
        },
...
export function getResourceStats() {
  const stats = {
    ...
    averageQuality:
      PDF_RESOURCES.reduce((sum, r) => sum + r.metadata.quality.overall, 0) /
      PDF_RESOURCES.length,
    totalDownloads: PDF_RESOURCES.reduce(
      (sum, r) => sum + r.analytics.downloadCount,
      0,
    ),
    totalViews: PDF_RESOURCES.reduce(
      (sum, r) => sum + r.analytics.viewCount,
      0,
    ),
    ...
    byLanguage: PDF_RESOURCES.reduce(
      (acc, r) => {
    ...
    topResources: {
      mostDownloaded: PDF_RESOURCES.sort(
        (a, b) => b.analytics.downloadCount - a.analytics.downloadCount,
      )
        .slice(0, 5)
        .map((r) => ({ resourceId: r.id, count: r.analytics.downloadCount })),

      highestRated: PDF_RESOURCES.sort(
        (a, b) => b.metadata.quality.overall - a.metadata.quality.overall,
      )
        .slice(0, 5)
        .map((r) => ({ resourceId: r.id, rating: r.metadata.quality.overall })),

      mostRecent: PDF_RESOURCES.sort(
 ...
 * 根据类别获取资源
 */
export function getPDFResourcesByCategory(
  category: ResourceCategory,
): PDFResource[] {
  return PDF_RESOURCES.filter(
...
export function getFeaturedResources(): PDFResource[] {
  return PDF_RESOURCES.filter(
    (r) => r.metadata.featured && r.status === "active",
  );
}
...
export function validateLegacyMappings(): { valid: boolean; errors: string[] } {
  ...
  for (const [legacyId, modernId] of Object.entries(LEGACY_ID_MAPPING)) {
    ...
    if (!resource) {
      errors.push(
        `Legacy mapping ${legacyId} -> ${modernId} points to non-existent resource`,
      );
    }
  }
