...
export class PrivacyProtectionManager {
  ...
  private maskPeriodData(data: any): any {
    if (Array.isArray(data.data)) {
      data.data = data.data.map((record: PeriodRecord) => ({
        ...
        // 保留疼痛等级但脱敏具体数值
        painLevel: record.painLevel ? this.maskPainLevel(record.painLevel) : null,
        // 保留流量类型但脱敏具体描述
  ...
  private maskNutritionData(data: any): any {
    if (Array.isArray(data.data)) {
      data.data = data.data.map((item: NutritionRecommendation) => ({
        ...
        // 保留营养素但脱敏具体含量
        nutrients: item.nutrients.map(nutrient => this.maskNutrient(nutrient))
      }));
    }
    return data;
  }
  ...
  private maskBenefit(benefit: string): string {
    const keywords = ['补血', '止痛', '调经', '安神', 'Replenish', 'Relieve', 'Improve'];
  ...
  private maskNutrient(nutrient: string): string {
    const nutrients = ['铁', '维生素C', '蛋白质', 'Iron', 'Vitamin C', 'Protein'];
   ...
   * 权限控制 - 检查导出权限
   */
  async checkExportPermission(dataType: ExportType, password?: string): Promise<boolean> {
    // 记录权限检查
   ...
   * 记录审计日志
   */
  private logAuditEvent(action: AuditLogEntry['action'], dataType: ExportType): void {
