...
export default function HistoryDataViewer() {
  ...
  const exportData = () => {
    ...
    link.href = url;
    link.download = `period_history_${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="predicted">{t('export.periodTypes.predicted')}</option>
                  <option value="ovulation">{t('export.periodTypes.ovulation')}</option>
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFilters(prev => ({ ...prev, painLevel: e.target.value as any }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                >
              ...
              <div>
                ...
                <select
                  value={filters.flow}
                  onChange={(e) => setFilters(prev => ({ ...prev, flow: e.target.value as any }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                >
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200">
              ...
              <th className="text-left py-3 px-4 font-medium text-gray-700">{t('history.type')}</th>
              <th className="text-left py-3 px-4 font-medium text-gray-700">{t('history.painLevel')}</th>
              <th className="text-left py-3 px-4 font-medium text-gray-700">{t('history.flow')}</th>
              <th className="text-left py-3 px-4 font-medium text-gray-700">{t('history.notes')}</th>
              <th className="text-left py-3 px-4 font-medium text-gray-700">{t('history.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.length === 0 ? (
              ...
              filteredData.map((record, index) => (
                <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <span className={`font-medium ${getPainLevelStyle(record.painLevel)}`}>
                      {record.painLevel || '-'}
                    </span>
                  </td>
                  <td className="py-3 px-4">
                    <span className={`font-medium ${getFlowStyle(record.flow)}`}>
            ...
            <div className="space-y-3">
              ...
              {selectedRecord.painLevel && (
                <div>
                  ...
                  <p className={`font-medium ${getPainLevelStyle(selectedRecord.painLevel)}`}>
                    {selectedRecord.painLevel}
                  </p>
                </div>
              )}
              ...
              {selectedRecord.flow && (
                <div>
                  ...
                  <p className={`font-medium ${getFlowStyle(selectedRecord.flow)}`}>
