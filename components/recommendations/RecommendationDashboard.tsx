...
export const RecommendationDashboard: React.FC<RecommendationDashboardProps> = ({
  ...
  const quickStats: QuickStats = useMemo(() => {
    ...
    const urgent = activeRecommendations.filter(r => r.priority === "urgent").length;
  ...
  const categoryInsights: CategoryInsight[] = useMemo(() => {
    const categories = [...new Set(recommendationHistory.items.map(item => item.content.category))];
    
    return categories.map(category => {
      const categoryItems = recommendationHistory.items.filter(item => item.content.category === category);
  ...
  // 渲染统计卡片
  const renderStatCard = (title: string, value: string | number, icon: React.ReactNode, color: string, trend?: "up" | "down") => (
    <div className={cn("p-6 bg-white rounded-xl border border-gray-200 shadow-sm", color)}>
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        >
          ...
          <span>{isGenerating ? t("generating") : t("refreshRecommendations")}</span>
        </button>
        ...
        <button
          ...
          className={cn(
            ...
            autoRefresh ? "bg-orange-600 text-white hover:bg-orange-700" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
          )}
        >
    ...
    return (
      <div className="flex items-center space-x-2 mb-6">
        ...
        {[
          ...
          <button
            key={range.value}
            onClick={() => setSelectedTimeRange(range.value as "week" | "month" | "all")}
            className={cn(
  ...
  return (
    <div className={cn("space-y-8", className)}>
      ...
      {todayHighlights.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">{t("todayHighlights")}</h2>
      ...
      {showAnalytics && categoryInsights.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">{t("categoryInsights")}</h2>
            <BarChart3 className="h-5 w-5 text-gray-400" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryInsights.slice(0, 6).map((insight) => (
              <div key={insight.category} className={cn("p-4 rounded-lg border", insight.color)}>
                ...
                <div className="text-sm text-gray-600">
                  <div>{t("count")}: {insight.count}</div>
                  <div>{t("effectiveness")}: {(insight.effectiveness * 100).toFixed(1)}%</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {showAnalytics && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* 完成进度 */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("weeklyProgress")}</h2>
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-600">
                <div className="flex items-center space-x-1">
                  ...
                  <span>{t("completed")}: {recommendationHistory.totalCompleted}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <Bookmark className="h-4 w-4 text-blue-600" />
                  <span>{t("saved")}: {recommendationHistory.totalSaved}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <Eye className="h-4 w-4 text-gray-600" />
                  <span>{t("viewed")}: {recommendationHistory.totalViewed}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* 活跃状态 */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">{t("activeStatus")}</h2>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">{t("activeRecommendations")}</span>
                <span className="font-semibold">{quickStats.activeRecommendations}</span>
              </div>
              <div className="space-y-2">
                {[
                  { priority: "urgent" as RecommendationPriority, color: "bg-red-500" },
                  { priority: "high" as RecommendationPriority, color: "bg-orange-500" },
                  { priority: "medium" as RecommendationPriority, color: "bg-blue-500" },
                  { priority: "low" as RecommendationPriority, color: "bg-gray-500" },
                ].map(({ priority, color }) => {
