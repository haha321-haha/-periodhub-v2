...
export default function PersonalizedRecommendationEngine({ 
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'treatment' | 'lifestyle' | 'prevention' | 'emergency'>('all');
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2 flex items-center">
            ...
            {locale === 'zh' ? 'AI个性化推荐引擎' : 'AI Personalized Recommendation Engine'}
          </h2>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {(['all', 'treatment', 'lifestyle', 'prevention', 'emergency'] as const).map(category => (
          <button
            key={category}
          ...
          >
            ...
            {category === 'treatment' && (locale === 'zh' ? '治疗' : 'Treatment')}
            {category === 'lifestyle' && (locale === 'zh' ? '生活方式' : 'Lifestyle')}
            {category === 'prevention' && (locale === 'zh' ? '预防' : 'Prevention')}
            {category === 'emergency' && (locale === 'zh' ? '紧急' : 'Emergency')}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredRecommendations.map((recommendation) => (
          <div key={recommendation.id} className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg p-6 border border-gray-200 hover:shadow-md transition-all duration-300">
            {/* 推荐头部 */}
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-2">
                {recommendation.type === 'treatment' && <Heart className="w-5 h-5 text-red-500" />}
                {recommendation.type === 'lifestyle' && <Activity className="w-5 h-5 text-green-500" />}
                {recommendation.type === 'prevention' && <Target className="w-5 h-5 text-blue-500" />}
                {recommendation.type === 'emergency' && <Zap className="w-5 h-5 text-orange-500" />}
              ...
              <div className="flex items-center space-x-2">
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  ...
                  {recommendation.priority === 'low' && (locale === 'zh' ? '低优先级' : 'Low Priority')}
                </span>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="text-center">
                <div className="text-lg font-bold text-blue-600">{Math.round(recommendation.effectiveness * 100)}%</div>
                <div className="text-xs text-gray-600">{locale === 'zh' ? '有效性' : 'Effectiveness'}</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-green-600">{Math.round(recommendation.confidence * 100)}%</div>
                <div className="text-xs text-gray-600">{locale === 'zh' ? '置信度' : 'Confidence'}</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-bold text-purple-600">{recommendation.scientificEvidence.studies}</div>
                <div className="text-xs text-gray-600">{locale === 'zh' ? '研究数量' : 'Studies'}</div>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {recommendations.reduce((sum, rec) => sum + rec.scientificEvidence.studies, 0)}
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {Math.round(recommendations.reduce((sum, rec) => sum + rec.scientificEvidence.effectiveness, 0) / recommendations.length * 100)}%
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {Math.round(recommendations.reduce((sum, rec) => sum + rec.scientificEvidence.safety, 0) / recommendations.length * 100)}%
            </div>
