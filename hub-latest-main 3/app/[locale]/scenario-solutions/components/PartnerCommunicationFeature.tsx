...
export default function PartnerCommunicationFeature({ locale }: PartnerCommunicationFeatureProps) {
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center mb-4">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
                  {isZh ? '💕 伴侣沟通专区 💕' : '💕 Partner Communication Zone 💕'}
                </h2>
                <p className="text-sm text-purple-600 font-medium">
                  {isZh ? '专为伴侣设计的理解与支持空间' : 'A caring space designed for partners to understand and support'}
                </p>
              </div>
            </div>
          ...
          <div className="w-full lg:w-1/3 mt-6 lg:mt-0">
            <div className="bg-white rounded-xl p-4 sm:p-6 shadow-lg border border-purple-100">
              ...
              <div className="space-y-2 sm:space-y-3">
                {[
                  { icon: '🧠', text: isZh ? '伴侣理解测试' : 'Partner Understanding Test' },
                  { icon: '🏋️', text: isZh ? '30天训练营' : '30-Day Training Camp' },
                  { icon: '💬', text: isZh ? '沟通技巧指导' : 'Communication Skills Guidance' },
                  { icon: '🎯', text: isZh ? '个性化建议' : 'Personalized Recommendations' },
                  { icon: '📖', text: isZh ? '情感支持手册' : 'Emotional Support Guide' }
                ].map((item, index) => (
