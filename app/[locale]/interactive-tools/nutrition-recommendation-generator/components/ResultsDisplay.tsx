/**
 * 结果展示组件 - 基于ziV1d3d的displayResults方法
 * 展示个性化营养建议结果
 */
        ...
        {results.items.map((data, index) => (
          <div key={index} className="result-card mb-6" style={{ animationDelay: data.delay }}>
            <h3 className="text-lg font-semibold mb-4 text-neutral-800 flex items-center gap-2">
              <svg className={`w-5 h-5 ${data.color}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <ul className="space-y-3 text-neutral-800">
              {data.items.map((item, itemIndex) => (
                <li key={itemIndex} className="flex items-start gap-3">
                  <svg className={`w-5 h-5 ${data.color} flex-shrink-0 mt-1`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    {data.icon === 'check-circle-2' && (
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    )}
                    {data.icon === 'x-circle' && (
