...
export interface RecommendationStoreActions {
  // 推荐生成
  generateRecommendations: (assessmentHistory: AssessmentHistory) => Promise<void>;
  ...
  // 反馈管理
  submitFeedback: (feedback: Omit<RecommendationFeedback, "id" | "timestamp">) => void;
  updateFeedback: (feedbackId: string, updates: Partial<RecommendationFeedback>) => void;
...
// 推荐系统Store类型
export type RecommendationStore = RecommendationStoreState & RecommendationStoreActions;

/**
 * 创建推荐系统Store
 */
export const createRecommendationStore = (): ZustandStoreApi<RecommendationStore> => {
  return create<RecommendationStore>()(
    persist(
      (set, get) => ({
        ...
        // 推荐生成
        generateRecommendations: async (assessmentHistory: AssessmentHistory) => {
          const store = get();
          ...
          try {
            ...
            const context: RecommendationContext = {
              ...
              lastAssessment: assessmentHistory.lastAssessmentDate 
                ? assessmentHistory.records.find(r => r.date === assessmentHistory.lastAssessmentDate)
                : assessmentHistory.records[assessmentHistory.records.length - 1],
            ...
            // 使用算法引擎生成推荐
            const result = await defaultRecommendationEngine.generateRecommendations(
              context,
            ...
            const updatedHistory = {
              items: [...store.recommendationHistory.items, ...result.recommendations],
              totalGenerated: store.recommendationHistory.totalGenerated + result.recommendations.length,
            ...
            set({ 
              error: error instanceof Error ? error.message : "生成推荐时发生错误",
              isGenerating: false 
            });
          }
        },
        ...
        dismissRecommendation: (recommendationId: string) => {
          const store = get();
          const updatedRecommendations = store.activeRecommendations.map(rec => 
            rec.id === recommendationId 
              ? { ...rec, status: "dismissed" as const, dismissedAt: new Date().toISOString() }
              : rec
          );

          const filteredRecommendations = updatedRecommendations.filter(rec => rec.status !== "dismissed");
        ...
        saveRecommendation: (recommendationId: string) => {
          ...
          const updatedRecommendations = store.activeRecommendations.map(rec => 
            rec.id === recommendationId 
              ? { ...rec, status: "saved" as const, savedAt: new Date().toISOString() }
              : rec
          );
        ...
        markAsViewed: (recommendationId: string) => {
          ...
          const updatedRecommendations = store.activeRecommendations.map(rec => 
            rec.id === recommendationId 
              ? { ...rec, status: "viewed" as const, viewedAt: new Date().toISOString() }
              : rec
          );
        ...
        markAsCompleted: (recommendationId: string) => {
          ...
          const updatedRecommendations = store.activeRecommendations.map(rec => 
            rec.id === recommendationId 
              ? { ...rec, status: "completed" as const, completedAt: new Date().toISOString() }
              : rec
          );
        ...
        // 反馈管理
        submitFeedback: (feedback: Omit<RecommendationFeedback, "id" | "timestamp">) => {
          const store = get();
        ...
        updateFeedback: (feedbackId: string, updates: Partial<RecommendationFeedback>) => {
          const store = get();
        ...
        updateContext: (context: Partial<RecommendationContext>) => {
          const store = get();
          set({
            currentContext: store.currentContext ? { ...store.currentContext, ...context } : context,
          });
        },
        ...
        importData: (data: string) => {
          try {
            ...
            // 验证数据结构
            if (importedData.activeRecommendations && importedData.recommendationHistory) {
              set({
                activeRecommendations: importedData.activeRecommendations || [],
        ...
        updateStatistics: () => {
          ...
          // 计算平均评分
          const ratingsWithValues = feedbacks.filter(f => f.rating !== undefined);
          ...
          const categories = [...new Set(history.items.map(item => item.content.category))];

          for (const category of categories) {
            const categoryItems = history.items.filter(item => item.content.category === category);
            const positiveFeedback = feedbacks.filter(f => 
              categoryItems.some(item => item.id === f.recommendationId) && 
              (f.type === "useful" || f.type === "helpful" || (f.rating && f.rating >= 4))
            );
            
            categoryEffectiveness[category] = categoryItems.length > 0 
          ...
          for (const priority of ["urgent", "high", "medium", "low"] as const) {
            const priorityItems = history.items.filter(item => item.priority === priority);
            const engagedItems = priorityItems.filter(item => 
              item.status === "viewed" || item.status === "saved" || item.status === "completed"
            );
            
            priorityEngagement[priority] = priorityItems.length > 0 
        ...
        generateReport: (dateRange?: { start: string; end: string }) => {
          ...
          const filteredHistory = store.recommendationHistory.items.filter(item => 
            filterByDate(item.generatedAt)
          );
          ...
          return {
            summary: {
              ...
              totalViews: filteredHistory.filter(item => item.status === "viewed").length,
              totalSaves: filteredHistory.filter(item => item.status === "saved").length,
              totalCompletions: filteredHistory.filter(item => item.status === "completed").length,
              totalFeedbacks: filteredFeedbacks.length,
            },
            breakdown: {
              byPriority: {
                urgent: filteredHistory.filter(item => item.priority === "urgent").length,
                high: filteredHistory.filter(item => item.priority === "high").length,
                medium: filteredHistory.filter(item => item.priority === "medium").length,
                low: filteredHistory.filter(item => item.priority === "low").length,
              },
              byCategory: filteredHistory.reduce((acc, item) => {
                acc[item.content.category] = (acc[item.content.category] || 0) + 1;
                return acc;
              }, {} as Record<string, number>),
              byType: filteredHistory.reduce((acc, item) => {
                acc[item.content.type] = (acc[item.content.type] || 0) + 1;
                return acc;
              }, {} as Record<string, number>),
            },
            engagement: {
              viewRate: filteredHistory.length > 0 ? filteredHistory.filter(item => item.status === "viewed").length / filteredHistory.length : 0,
              saveRate: filteredHistory.length > 0 ? filteredHistory.filter(item => item.status === "saved").length / filteredHistory.length : 0,
              completionRate: filteredHistory.length > 0 ? filteredHistory.filter(item => item.status === "completed").length / filteredHistory.length : 0,
            },
            feedback: {
              averageRating: store.statistics.averageRating,
              satisfactionRate: filteredFeedbacks.filter(f => f.type === "useful" || f.type === "helpful").length / Math.max(1, filteredFeedbacks.length),
            },
