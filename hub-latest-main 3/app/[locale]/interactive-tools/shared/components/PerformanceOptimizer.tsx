...
export default function PerformanceOptimizer({
  ...
  const optimizePerformance = useCallback(async () => {
    ...
    const steps = [
      { name: locale === 'zh' ? '代码分割优化...' : 'Code splitting optimization...', duration: 800 },
      { name: locale === 'zh' ? '懒加载实现...' : 'Implementing lazy loading...', duration: 600 },
      { name: locale === 'zh' ? '缓存策略优化...' : 'Cache strategy optimization...', duration: 700 },
      { name: locale === 'zh' ? '内存使用优化...' : 'Memory usage optimization...', duration: 500 },
      { name: locale === 'zh' ? '网络请求优化...' : 'Network request optimization...', duration: 600 },
      { name: locale === 'zh' ? '渲染性能优化...' : 'Rendering performance optimization...', duration: 500 },
      { name: locale === 'zh' ? '完成优化' : 'Optimization complete', duration: 300 }
    ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-6 border border-purple-200">
          ...
          <div className="space-y-3">
            <div className="flex items-center text-sm text-purple-700">
              <CheckCircle className="w-4 h-4 mr-2 text-green-600" />
              {locale === 'zh' ? '路由级别代码分割' : 'Route-level code splitting'}
            </div>
            <div className="flex items-center text-sm text-purple-700">
              <CheckCircle className="w-4 h-4 mr-2 text-green-600" />
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-6 border border-green-200">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center text-sm text-green-700">
              <CheckCircle className="w-4 h-4 mr-2 text-green-600" />
              {locale === 'zh' ? '浏览器缓存优化' : 'Browser cache optimization'}
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-6 border border-orange-200">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center text-sm text-orange-700">
              <CheckCircle className="w-4 h-4 mr-2 text-green-600" />
              {locale === 'zh' ? '预加载关键资源' : 'Preload critical resources'}
            </div>
            <div className="flex items-center text-sm text-orange-700">
              <CheckCircle className="w-4 h-4 mr-2 text-green-600" />
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {locale === 'zh' ? '当前包大小2.4MB，建议优化到1.7MB以下' : 'Current bundle size 2.4MB, recommend optimizing to under 1.7MB'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {locale === 'zh' ? '当前加载时间1.8秒，建议优化到1.1秒以下' : 'Current load time 1.8s, recommend optimizing to under 1.1s'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {locale === 'zh' ? '当前缓存命中率78%，建议提升到90%以上' : 'Current cache hit rate 78%, recommend improving to over 90%'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h4 className="font-semibold text-gray-900">
                  {locale === 'zh' ? '网络请求优化' : 'Network Request Optimization'}
                </h4>
                <p className="text-sm text-gray-600">
                  {locale === 'zh' ? '当前网络请求12个，建议减少到6个以下' : 'Current network requests 12, recommend reducing to under 6'}
                </p>
              </div>
            </div>
          </div>
