...
const RULES: LunaRule[] = [
    {
        keywords: ['pain', 'cramp', 'hurt', 'ache', 'sore', 'discomfort', '痛', '疼', '痛经', '缓解', '减轻', '如何缓解'],
        responseKey: 'responses.pain',
        links: [
            { labelKey: 'links.openPainTracker', url: "/interactive-tools/pain-tracker" },
            { labelKey: 'links.reliefMethods', url: "/health-guide/relief-methods" },
            { labelKey: 'links.understandingPain', url: "/health-guide/understanding-pain" }
        ]
    },
    {
        keywords: ['cycle', 'period', 'late', 'bleeding', 'flow', 'menstruation', '周期', '月经', '追踪', '追踪周期'],
        responseKey: 'responses.cycle',
        links: [{ labelKey: 'links.openCycleTracker', url: "/interactive-tools/cycle-tracker" }]
    },
    {
        keywords: ['privacy', 'data', 'store', 'save', 'security', '隐私', '数据', '安全', '数据安全'],
        responseKey: 'responses.privacy',
    },
    {
        keywords: ['guide', 'article', 'read', 'info', 'learn', 'download', '指南', '文章', '下载', '健康指南'],
    ...
    {
        keywords: ['mood', 'sad', 'angry', 'happy', 'emotion', '情绪', '心情', '情感'],
    ...
    {
        keywords: ['iud', 'birth control', 'contraception', 'coil', '节育器', '避孕', '环'],
        responseKey: 'responses.contraception',
        links: [{ labelKey: 'links.healthGuides', url: "/health-guide" }]
    },
    {
        keywords: ['fertility', 'pregnant', 'baby', 'conception', 'ovulation', '备孕', '怀孕', '排卵', '生宝宝'],
        responseKey: 'responses.fertility',
        links: [{ labelKey: 'links.openCycleTracker', url: "/interactive-tools/cycle-tracker" }]
    },
    {
        keywords: ['menopause', 'hot flash', 'aging', 'perimenopause', '绝经', '更年期', '围绝经', '潮热', '衰老'],
        responseKey: 'responses.menopause',
        links: [{ labelKey: 'links.healthGuides', url: "/health-guide" }]
    },
    {
        keywords: ['sleep', 'insomnia', 'tired', 'fatigue', '睡眠', '失眠', '累', '疲劳'],
        responseKey: 'responses.sleep',
        links: [{ labelKey: 'links.reliefMethods', url: "/health-guide/relief-methods" }]
    }
];
...
export const processLunaQuery = (query: string, locale: 'en' | 'zh' = 'en'): LunaResponse => {
    ...
    const defaultKey = DEFAULT_RESPONSE_KEYS[Math.floor(Math.random() * DEFAULT_RESPONSE_KEYS.length)];
    return {
        text: defaultKey,
 ...
 */
export function getSuggestedQuestions(context?: SuggestedQuestionsContext): string[] {
    const locale = context?.locale || (typeof window !== 'undefined'
        ? (window.location.pathname.includes('/zh') ? 'zh' : 'en')
        : 'en');

    const questionsPool = locale === 'zh' ? AGE_BASED_QUESTIONS.zh : AGE_BASED_QUESTIONS.en;
