  ...
  const generatePainLevelChart = (): ChartData => {
    ...
    return {
      ...
      datasets: [{
        ...
        data: counts,
        backgroundColor: [
          '#e8f5e8', '#d4edda', '#c3e6cb', '#b8dfcc', '#a8d5ba',
          '#f8d7da', '#f5c6cb', '#f1b0b7', '#ed9ca3', '#e8888f'
        ],
        borderColor: [
          '#28a745', '#28a745', '#28a745', '#28a745', '#28a745',
          '#dc3545', '#dc3545', '#dc3545', '#dc3545', '#dc3545'
        ],
        borderWidth: 1
      }]
    };
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {analysis.currentPhase && (
            <div className="bg-white rounded-lg p-3">
              ...
              <p className="text-xs text-gray-500 mt-1">{analysis.phaseProgress}%</p>
            </div>
          )}
        </div>
    ...
    <div className="bg-white rounded-xl shadow-sm border border-neutral-100 p-6">
      ...
      <div className="mb-4">
        <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            { id: 'overview', label: t('charts.tabs.overview'), icon: BarChart3 },
            { id: 'cycle-length', label: t('charts.tabs.cycleLength'), icon: TrendingUp },
            { id: 'pain-level', label: t('charts.tabs.painLevel'), icon: Activity },
            { id: 'flow-type', label: t('charts.tabs.flowType'), icon: PieChart }
          ].map((tab) => (
            <button
              key={tab.id}
