...
import { NutritionRecommendation, TCMNature } from "../types";
...
export function checkFoodCompatibility(
  ...
  // 统计各性质的食物数量
  const natureCount: Record<TCMNature, number> = {
    warm: 0,
    cool: 0,
    neutral: 0,
  };
  ...
  foods.forEach((food) => {
    natureCount[food.tcmNature] = (natureCount[food.tcmNature] || 0) + 1;
  });

  // 检查寒热冲突
  if (natureCount.warm > 0 && natureCount.cool > 0) {
    const warmFoods = foods.filter((f) => f.tcmNature === "warm");
    const coolFoods = foods.filter((f) => f.tcmNature === "cool");
...
export function optimizeFoodDistribution(
  ...
  const warmFoods = foods.filter((f) => f.tcmNature === "warm");
  const coolFoods = foods.filter((f) => f.tcmNature === "cool");
  const neutralFoods = foods.filter((f) => f.tcmNature === "neutral");
