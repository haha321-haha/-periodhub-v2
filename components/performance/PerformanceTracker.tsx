    ...
    const trackScriptPerformance = () => {
      const observer = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          // 监控关键第三方脚本
          if (entry.name.includes('googletagmanager') || 
            ...
            // 性能警告
            if (loadTime > 1000) {
              console.warn(`⚠️ Slow script detected: ${entry.name} took ${loadTime}ms`);
            }
          }
        }
      });
...
export function ScriptOptimizationSuggestions() {
  useEffect(() => {
    ...
    const analyzeScripts = () => {
      ...
      // 提供优化建议
      if (scripts.length > 5) {
        console.log('💡 Optimization suggestion: Consider reducing the number of third-party scripts');
      }
      
      if (scripts.some(s => !s.name.includes('lazyOnload'))) {
        console.log('💡 Optimization suggestion: Consider using lazyOnload for non-critical scripts');
      }
    };
