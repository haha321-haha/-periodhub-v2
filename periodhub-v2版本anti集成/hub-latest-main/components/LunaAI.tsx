...
const LunaAI: React.FC = () => {
            ...
            if (storedAge) {
                ...
                try {
                    ...
                    if (userProfile) {
                        ...
                        else if (parsed.birthYear) userAge = new Date().getFullYear() - parseInt(parsed.birthYear, 10);
                    }
                } catch {
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
            {/* Chat Window */}
            <div
                className={`bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-purple-100 dark:border-purple-900 w-80 sm:w-96 h-[500px] flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4 pointer-events-none'}`}
                role="dialog"
            ...
            >
                ...
                <div className="bg-gradient-to-r from-purple-600 to-pink-500 p-4 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-2 text-white">
                        ...
                        <div>
                            ...
                            <span className="text-xs text-purple-100 opacity-90">{t('tagline')}</span>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-900 custom-scrollbar">
                                ...
                                {msg.links && (
                                    <div className="mt-3 space-y-1">
                                        {msg.links.map((link, lIdx) => {
                                            ...
                                            const urlAlreadyHasLocale = link.url.match(/^\/(zh|en)\//);
                    ...
                    {messages.length > 0 && messages[messages.length - 1].sender === 'luna' && !isTyping && suggestedQuestions.length > 0 && (
                ...
                {/* Input Area */}
                <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-slate-800 border-t border-gray-100 dark:border-slate-700 shrink-0">
                    <div className="relative flex items-center">
                        <input
                            type="text"
                            value={inputValue}
                            onChange={(e) => setInputValue(e.target.value)}
                            placeholder={!isPro && dailyMessageCount >= DAILY_LIMIT ? t('input.limitReached') : t('input.placeholder')}
                    ...
                    <div className="text-[10px] text-gray-400 dark:text-gray-500 text-center mt-2 flex justify-center items-center gap-1">
                        {!isPro && <span>{t('footer.freeMessagesLeft', { count: DAILY_LIMIT - dailyMessageCount })}</span>}
