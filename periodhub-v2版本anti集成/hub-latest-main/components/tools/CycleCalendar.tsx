...
export const CycleCalendar = () => {
  ...
  const calculatePhase = (data: CycleData): { phase: CyclePhase; dayInCycle: number; nextPeriodDays: number } => {
    const lastPeriod = parseISO(data.lastPeriodDate);
    if (!isValid(lastPeriod)) return { phase: 'luteal', dayInCycle: 1, nextPeriodDays: 0 };
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('cycleLengthLabel')}</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('periodLengthLabel')}</label>
              <input
                type="number"
                value={tempData.periodLength}
                onChange={(e) => setTempData({ ...tempData, periodLength: parseInt(e.target.value) || 5 })}
                className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-purple-500 outline-none"
              />
            </div>
          </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
      ...
      <div className={`rounded-xl p-4 mb-6 ${currentConfig.bg}`}>
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className={`font-semibold ${currentConfig.color} mb-1`}>{t('todaysFocus')}</h4>
