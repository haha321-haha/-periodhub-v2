...
import { MessageCircle, X, Send, ChevronRight, Sparkles, Lock } from 'lucide-react';
import { trackEvent } from '../utils/analytics';
import { processLunaQuery, LunaResponse } from '../utils/lunaEngine';
...
const LunaAI: React.FC = () => {
  ...
  useEffect(() => {
    const checkHoliday = () => {
      ...
      if (month === 2 && day === 14) {
        setHolidayMode({ icon: '💝', message: "Happy Valentine's Day! Love yourself! 💕" });
  ...
  const triggerEmojiRain = () => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      setTimeout(() => { if (container.contains(el)) container.removeChild(el); }, 4000);
    }
  };

  const handleUpgrade = () => {
    setIsPro(true);
    localStorage.setItem('luna_is_pro', 'true');
    trackEvent('luna_upgrade_success');

    setMessages(prev => [...prev, {
      sender: 'luna',
      text: "You're now a Pro member! 🌟 Enjoy unlimited chats and premium insights.",
      timestamp: Date.now()
    }]);
    triggerEmojiRain();
  };
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    if (!isPro && dailyMessageCount >= DAILY_LIMIT) {
      setMessages(prev => [...prev, { sender: 'user', text: inputValue.trim(), timestamp: Date.now() }]);
      setInputValue('');

      setTimeout(() => {
        setMessages(prev => [...prev, {
          sender: 'luna',
          text: "You've reached your daily free message limit. Upgrade to Pro for unlimited chats and deeper health insights! 💜",
          timestamp: Date.now(),
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      {/* Chat Window */}
      <div
        className={`bg-white rounded-2xl shadow-2xl border border-purple-100 w-80 sm:w-96 h-[500px] flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4 pointer-events-none'}`}
        role="dialog"
      ...
      >
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-500 p-4 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2 text-white">
            <span className="text-2xl" role="img" aria-label="Luna Icon">{holidayMode?.icon || '🌙'}</span>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold leading-none">Luna AI</h3>
                {isPro && <span className="bg-white/20 text-xs px-2 py-0.5 rounded-full font-medium flex items-center gap-1"><Sparkles size={10} /> PRO</span>}
              </div>
              <span className="text-xs text-purple-100 opacity-90">Always here to help</span>
            </div>
          </div>
          <button
            ...
            <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div
                className={`max-w-[85%] rounded-2xl p-3 text-sm shadow-sm ${msg.sender === 'user'
                    ...
                    : msg.isUpgradePrompt
                      ? 'bg-gradient-to-br from-purple-50 to-pink-50 border border-purple-200 rounded-bl-none'
                  ...
                  <div className="mt-3 space-y-1">
                    {msg.links.map((link, lIdx) => (
                      <a
                        key={lIdx}
                        href={link.url}
                        className={`block text-xs font-bold py-1 px-2 rounded transition-colors flex items-center justify-between ${msg.sender === 'user'
        ...
        <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-gray-100 shrink-0">
          <div className="relative flex items-center">
            <input
              ...
              placeholder={!isPro && dailyMessageCount >= DAILY_LIMIT ? "Daily limit reached" : "Type a message..."}
              disabled={!isPro && dailyMessageCount >= DAILY_LIMIT}
              className="w-full pl-4 pr-12 py-3 bg-gray-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            />
            <button
              type="submit"
              disabled={!inputValue.trim() || (!isPro && dailyMessageCount >= DAILY_LIMIT)}
          ...
          <div className="text-[10px] text-gray-400 text-center mt-2 flex justify-center items-center gap-1">
            {!isPro && <span>{DAILY_LIMIT - dailyMessageCount} free messages left today.</span>}
      ...
      >
        <div className="relative">
          <span className={`text-3xl transition-all duration-300 ${isOpen ? 'scale-0 opacity-0' : 'scale-100 opacity-100'}`} role="img" aria-label="Moon icon">{holidayMode?.icon || '🌙'}</span>
          <X className={`text-white w-8 h-8 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`} />
          {!isOpen && (
