...
export function analyzeRecommendationQuality(
  ...
  const metrics: QualityMetrics = {
    relevance: calculateRelevance(result, userData),
...
function calculateDiversity(result: RecommendationResult): number {
  ...
  const categoryDiversity = (categories.size / result.recommendations.length) * 100;
...
function calculateCoverage(result: RecommendationResult): number {
  ...
  const coveredCategories = new Set(result.recommendations.map(r => r.category));
...
export function validateRecommendationReasonableness(
  ...
  // 检查推荐分数是否合理
  const lowScoreCount = result.recommendations.filter(r => r.score < 30).length;
  if (lowScoreCount > 0) {
