...
import {
  ...
  withMemoization,
  PerformanceMonitor,
} from "../shared/components/LazyLoading";
...
const RecommendationCard = withMemoization<{
  ...
  >
    ...
    <p className="text-sm text-purple-600 mb-3">
      <strong>{locale === "zh" ? "推荐原因：" : "Why recommended: "}</strong>
      {recommendation.reason}
    </p>
    <p className="text-sm text-purple-500 mb-3">
      <strong>{locale === "zh" ? "时间框架：" : "Timeframe: "}</strong>
...
export default function SymptomAssessmentTool({
  ...
  const { debounce, memoize, getCachedValue, setCachedValue, startMonitoring } =
                        ...
                        <span className="text-xl font-bold text-blue-800">
                          ...
                          <span className="text-base font-medium text-blue-700 ml-2">
                            ...
                            {(() => {
                              ...
                              if (value <= 8)
                                return t("painScale.levels.severe");
