...
export default function AdvancedCycleAnalysis() {
  ...
  // 辅助函数
  const getMostCommonFlow = (records: PeriodRecord[]): 'light' | 'medium' | 'heavy' => {
    const flows = records.map(r => r.flow).filter(Boolean);
    const counts = { light: 0, medium: 0, heavy: 0 };
    flows.forEach(flow => counts[flow!]++);
    return Object.entries(counts).reduce((a, b) => counts[a[0] as keyof typeof counts] > counts[b[0] as keyof typeof counts] ? a : b)[0] as 'light' | 'medium' | 'heavy';
  };
          ...
          <div key={index} className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-gray-600">{t('analysis.commonFlow')}</div>
              </div>
            </div>
          </div>
        ))}
  ...
  const renderComparisonAnalysis = () => {
    ...
    const getTrendIcon = () => {
      switch (comparisonData.trend) {
        ...
        case 'down': return <TrendingUp className="w-5 h-5 text-green-500 rotate-180" />;
    ...
    const getTrendText = () => {
      switch (comparisonData.trend) {
        ...
        default: return t('analysis.trendStable');
      }
    };
      ...
      <div className="mb-6">
        <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            ...
            { id: 'trends', label: t('analysis.tabs.trends'), icon: TrendingUp },
            { id: 'comparison', label: t('analysis.tabs.comparison'), icon: Target },
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-blue-600">{Math.round(analysis.averageCycleLength)} {t('charts.days')}</p>
                  </div>
              ...
              <div className="bg-green-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600 mb-1">{t('analysis.avgPeriodLength')}</p>
                    <p className="text-lg font-semibold text-green-600">{Math.round(analysis.averagePeriodLength)} {t('charts.days')}</p>
                  </div>
              ...
              <div className="bg-red-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-red-600">{statistics?.averagePainLevel.toFixed(1)}</p>
                  </div>
              ...
              <div className="bg-purple-50 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-lg font-semibold text-purple-600">{analysis.confidence.toFixed(0)}%</p>
                  </div>
