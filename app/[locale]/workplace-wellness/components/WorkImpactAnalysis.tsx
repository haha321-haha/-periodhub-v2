...
export default function WorkImpactAnalysis() {
  ...
  const calculateProductivity = (painLevel: number, efficiency: number): number => {
    // 综合计算生产力
  ...
  const getCommonAdjustments = (data: WorkImpactData[]): WorkAdjustment[] => {
    const adjustments = data.map(d => d.adjustment).filter(Boolean) as WorkAdjustment[];
    const counts = adjustments.reduce((acc, adj) => {
      acc[adj] = (acc[adj] || 0) + 1;
      return acc;
  ...
  const renderWorkOverview = () => {
    ...
    const avgEfficiency = workData.reduce((sum, d) => sum + d.workEfficiency, 0) / workData.length;
    const avgPain = workData.reduce((sum, d) => sum + d.painLevel, 0) / workData.length;
    const avgProductivity = workData.reduce((sum, d) => sum + d.productivity, 0) / workData.length;
    const adjustmentRate = (workData.filter(d => d.adjustment !== null).length / workData.length) * 100;
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-blue-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold text-blue-600">{avgEfficiency.toFixed(1)}%</p>
              </div>
          ...
          <div className="bg-red-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('workAnalysis.avgPain')}</p>
                <p className="text-lg font-semibold text-red-600">{avgPain.toFixed(1)}</p>
              </div>
          ...
          <div className="bg-green-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('workAnalysis.avgProductivity')}</p>
                <p className="text-lg font-semibold text-green-600">{avgProductivity.toFixed(1)}</p>
              </div>
          ...
          <div className="bg-purple-50 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('workAnalysis.adjustmentRate')}</p>
                <p className="text-lg font-semibold text-purple-600">{adjustmentRate.toFixed(1)}%</p>
              </div>
            ...
            {workData.slice(-5).map((data, index) => (
              <div key={index} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <span className="text-sm text-blue-600">{data.workEfficiency.toFixed(0)}%</span>
                  <span className="text-sm text-red-600">{data.painLevel.toFixed(1)}</span>
                  <span className="text-sm text-green-600">{data.productivity.toFixed(0)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {patterns.map((pattern, index) => (
          <div key={index} className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-semibold text-gray-800">{t(`nutrition.phases.${pattern.phase}`)}</h4>
              <div className="flex items-center space-x-2">
                {pattern.productivityTrend === 'up' && <TrendingUp className="w-4 h-4 text-green-500" />}
                {pattern.productivityTrend === 'down' && <TrendingDown className="w-4 h-4 text-red-500" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {pattern.commonAdjustments.slice(0, 2).map((adj, adjIndex) => (
                    <span key={adjIndex} className="px-2 py-1 bg-white rounded text-xs text-gray-700">
                  ...
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-xs text-gray-600">{data.workEfficiency.toFixed(0)}%</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <div className="w-16 bg-gray-200 rounded-full h-2">
                          <div
                        ...
                        <span className="text-xs text-gray-600">{data.painLevel.toFixed(1)}</span>
                      </div>
                    </div>
                  </div>
                ))}
                    ...
                    <div className="flex items-center space-x-2">
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${(data.productivity / 100) * 100}%` }}
                        />
                      </div>
                      <span className="text-xs text-gray-600">{data.productivity.toFixed(0)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div key={index} className={`rounded-lg p-4 ${
            ...
            <div className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800">{t('workAnalysis.recommendation')}: {insight.recommendation}</p>
              </div>
            </div>
          </div>
        ))}
    ...
    <div className="bg-white rounded-xl shadow-sm border border-neutral-100 p-6">
      ...
      <div className="mb-6">
        <div className="flex space-x-1 bg-gray-100 rounded-lg p-1">
          {[
            { id: 'overview', label: t('workAnalysis.tabs.overview'), icon: BarChart3 },
            { id: 'patterns', label: t('workAnalysis.tabs.patterns'), icon: PieChart },
            { id: 'productivity', label: t('workAnalysis.tabs.productivity'), icon: Target },
            { id: 'insights', label: t('workAnalysis.tabs.insights'), icon: Users }
          ].map((tab) => (
            <button
              key={tab.id}
