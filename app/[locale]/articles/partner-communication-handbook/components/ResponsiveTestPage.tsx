...
export default function ResponsiveTestPage({ locale }: ResponsiveTestPageProps) {
  const { screenSize, dimensions } = useResponsive();
  const { isMobile, isTablet, isDesktop, isTouchDevice, minTouchTarget } = useMobileOptimization();
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <ResponsiveGrid mobileCols={1} tabletCols={2} desktopCols={3}>
            ...
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <p className="text-green-700">{dimensions.width} × {dimensions.height}</p>
            </div>
              ...
              {[1, 2, 3].map((item) => (
                <div key={item} className="text-center">
                  ...
                  <p className="text-sm text-gray-600 mt-2">
                    {locale === 'zh' ? `测试图片 ${item}` : `Test Image ${item}`}
                  </p>
                </div>
              ))}
            </ResponsiveGrid>
            ...
            <form className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={locale === 'zh' ? '请输入您的消息' : 'Enter your message'}
                />
              </div>
              <button
                type="submit"
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mt-8">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">{locale === 'zh' ? '最小触控目标' : 'Min Touch Target'}:</span>
              <span className="font-medium">{minTouchTarget}px</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">{locale === 'zh' ? '当前断点' : 'Current Breakpoint'}:</span>
              <span className="font-medium">{screenSize}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">{locale === 'zh' ? '触控优化' : 'Touch Optimization'}:</span>
              <span className="font-medium">{isTouchDevice ? '已启用' : '未启用'}</span>
            </div>
          </div>
        </div>
      </div>
