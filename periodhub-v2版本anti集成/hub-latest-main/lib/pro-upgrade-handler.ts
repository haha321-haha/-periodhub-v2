...
export function handleProUpgrade(options: ProUpgradeOptions = {}) {
  ...
  console.log('🔓 Pro升级处理函数被调用:', { plan, painPoint, assessmentScore, source, email });
  ...
  if (email) {
    ...
    // 验证订阅状态
    verifySubscriptionWithCache(email).then(subscription => {
        ...
        if (!response.ok) {
          ...
          return response.text().then(text => {
            ...
            if (response.status === 401) {
              const errorMsg = errorData?.error || (errorData as { errors?: Array<{ detail?: string }> })?.errors?.[0]?.detail || 'Unauthenticated';
              console.error('❌ 支付 API 401 未认证错误:', errorData);
              throw new Error(`支付认证失败 (401): ${errorMsg}。请检查 API 密钥配置。`);
            }
            
            console.error('❌ 支付请求失败:', errorData);
            const errorMessage = errorData?.error || (errorData as { errors?: Array<{ detail?: string }> })?.errors?.[0]?.detail || `API error: ${response.status}`;
