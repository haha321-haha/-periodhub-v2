...
const NEW_PDF_INFO: Record<
  string,
  { title: { zh: string; en: string }; filename: string }
> = {
...
export default function PDFPreviewPage({
  ...
  useEffect(() => {
    ...
    fetch(fetchUrl)
      .then((response) => {
        ...
        if (!response.ok) {
          throw new Error(
            `HTTP ${response.status}: ${response.statusText} - File not found: ${htmlFilename}`,
          );
        }
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text:
        locale === "zh"
          ? "推荐这个有用的经期健康资源"
          : "Recommended period health resource",
  ...
  if (previewContent) {
    ...
    const fullVersionIncludes =
      previewContent.fullVersionIncludes[currentLocale];
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  title={
                    locale === "zh" ? "打印或保存为PDF" : "Print or Save as PDF"
                  }
                >
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                <span className="flex items-center">
                  ...
                  {locale === "zh" ? "预计使用时间" : "Estimated Time"}:{" "}
                  {estimatedTime}
                </span>
                <span className="flex items-center">
            ...
            <div className="mb-8">
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-lg">
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {fullVersionIncludes.map((item: string, index: number) => (
