...
const TEXTS = {
  ...
  en: {
    ...
    subtitle: 'Based on your assessment results, we provide the following personalized recommendations',
    ...
    disclaimer: 'These recommendations are for reference only and do not constitute medical diagnosis. Please seek medical attention for severe symptoms.',
    levels: {
      mild: 'Mild Impact',
...
export default function ProfessionalAdvice({
    ...
    if (score <= 60) {
      ...
      return {
        ...
        medical: isZh ? [
          ...
          'Consider traditional Chinese medicine or acupuncture',
  ...
  return (
    <div className="mt-12 p-8 bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl border-2 border-purple-200">
      ...
      {/* 评分结果 */}
      <div className={`p-6 rounded-xl ${impactLevel.bgColor} border-2 ${impactLevel.borderColor} mb-8`}>
        <div className="text-center">
          <p className="text-sm font-medium text-gray-600 mb-2">{t.impactLevel}</p>
          <h3 className={`text-2xl font-bold ${impactLevel.color} mb-2`}>
            {impactLevel.level}
          </h3>
          <p className="text-sm text-gray-600">
            {t.yourScore}: <span className={`font-bold ${impactLevel.color}`}>{impactScore}/100</span>
          </p>
        </div>
      </div>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-800 space-y-1 mb-3">
                <li>• {locale === 'zh' ? '多维度影响分析' : 'Multi-dimensional impact analysis'}</li>
                <li>• {locale === 'zh' ? '更具体的改善建议' : 'More specific improvement recommendations'}</li>
                <li>• {locale === 'zh' ? '心理和社交支持建议' : 'Psychological and social support advice'}</li>
              </ul>
              <button
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <ul className="text-sm text-purple-800 space-y-1 mb-3">
                <li>• {locale === 'zh' ? '临床级别的专业评估' : 'Clinical-level professional assessment'}</li>
                <li>• {locale === 'zh' ? '就医指导和专科推荐' : 'Medical guidance and specialist referrals'}</li>
                <li>• {locale === 'zh' ? '治疗方案建议' : 'Treatment plan recommendations'}</li>
                <li>• {locale === 'zh' ? '长期管理计划' : 'Long-term management plan'}</li>
              </ul>
              <button
              ...
              >
                {locale === 'zh' ? '升级到医疗专业版' : 'Upgrade to Medical Professional'}
              </button>
            </div>
          </div>
        </div>
