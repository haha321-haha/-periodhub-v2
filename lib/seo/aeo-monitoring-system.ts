 ...
 * 解析日志文件生成趋势数据
 */
export function parseValidationLog(logPath: string = 'logs/schema-validation.log'): TrendData {
...
function generateTrendChart(dailyTrends: TrendData['dailyTrends']): string {
  ...
  const scorePath = dailyTrends.map((trend, index) => {
    const x = padding + (index * (chartWidth - 2 * padding) / (dailyTrends.length - 1));
 ...
 * 生成常见问题表格
 */
function generateCommonIssuesTable(commonIssues: TrendData['commonIssues']): string {
 ...
 * 更新监控报告
 */
export function updateMonitoringReport(reportPath: string = 'reports/AEO-Monitoring-Report.html'): boolean {
  try {
    // 解析趋势数据
