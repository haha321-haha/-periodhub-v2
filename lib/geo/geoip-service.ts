...
export async function getLocationFromIP(ip?: string): Promise<GeoLocation> {
  ...
  try {
    ...
    logError('[GEO] Failed to get location from service', error, 'GeoIPService');
    // 返回默认值
...
async function getLocationFromMaxMind(ip?: string): Promise<GeoLocation> {
  ...
  const response = await fetch(url, {
    headers: {
      'Authorization': `Basic ${Buffer.from(`${process.env.MAXMIND_ACCOUNT_ID}:${apiKey}`).toString('base64')}`,
      'Accept': 'application/json',
    },
  });
...
export function optimizeContentForLocation(
  ...
  // 根据国家代码优化内容
  const countryOptimizations: Record<string, {
    preferredLanguage: string;
    currency?: string;
    dateFormat?: string;
  }> = {
