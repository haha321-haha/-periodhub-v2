...
import { WebVitalsMonitor, ErrorMonitor, UserBehaviorMonitor } from './monitoring';
...
export class FinalValidator {
    ...
    try {
      ...
      if (!resultsContainer) {
        return {
          ...
          status: 'warning',
          message: 'Results container not found (may be normal if no results generated)'
        };
      }
    ...
    try {
      ...
      if (!noSelectionElement) {
        return {
          ...
          status: 'warning',
          message: 'No selection state not found (may be normal if selections made)'
        };
      }
  ...
  private async validateNutritionData(): Promise<ValidationResult> {
    try {
      ...
      const tcmConstitutions = Object.keys(nutritionData.tcmConstitutionData);

      if (menstrualPhases.length === 0 || healthGoals.length === 0 || tcmConstitutions.length === 0) {
        return {
          category: 'Nutrition Data',
          status: 'fail',
      ...
      return {
        category: 'Nutrition Data',
        status: 'pass',
        message: `Found ${menstrualPhases.length} menstrual phases, ${healthGoals.length} health goals, ${tcmConstitutions.length} TCM constitutions`
      };
    } catch (error) {
      return {
        category: 'Nutrition Data',
  ...
  private async validateWebVitals(): Promise<ValidationResult> {
    try {
      ...
      return {
        category: 'Web Vitals',
        status: 'pass',
        message: `Web Vitals monitoring active with ${Object.keys(metrics).length} metrics`
      };
    } catch (error) {
      return {
        category: 'Web Vitals',
  ...
  // 生成摘要
  private generateSummary(): { total: number; passed: number; failed: number; warnings: number } {
