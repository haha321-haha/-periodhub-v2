...
export default function RealDataSystemTest() {
  ...
  const testAPIEndpoints = async () => {
    try {
      ...
      try {
        ...
        (results as any).sessionAPI = {
          success: sessionResponse.ok,
          status: sessionResponse.status,
          data: sessionData
        };
      } catch (error) {
        (results as any).sessionAPI = { success: false, error: (error as Error).message };
      }
      ...
      try {
        ...
        (results as any).feedbackAPI = {
          success: feedbackResponse.ok,
          status: feedbackResponse.status,
          data: feedbackData
        };
      } catch (error) {
        (results as any).feedbackAPI = { success: false, error: (error as Error).message };
      }
  ...
  const generateTestReport = () => {
    const allTests = Object.values(testResults).filter(r => r && typeof r === 'object' && 'success' in r) as any[];
    ...
    return {
      ...
      successRate: allTests.length > 0 ? (successfulTests.length / allTests.length * 100).toFixed(1) + '%' : '0%',
      overallStatus: failedTests.length === 0 ? 'PASS' : 'PARTIAL',
      summary: {
        dataCollection: testResults.dataCollection?.success ? '✅ 正常' : '❌ 异常',
        abTestAssignment: testResults.abTestAssignment?.success ? '✅ 正常' : '❌ 异常',
          ...
          <div className="space-y-6">
            ...
            {testResults.dataCollection && (
              <div className="border rounded-lg p-4">
                ...
                <div className={`p-3 rounded ${testResults.dataCollection.success ? 'bg-green-50' : 'bg-red-50'}`}>
                  <p className={testResults.dataCollection.success ? 'text-green-700' : 'text-red-700'}>
                    {testResults.dataCollection.message}
                  </p>
                  {testResults.feedbackId && (
                    <p className="text-sm text-gray-600 mt-2">反馈ID: {testResults.feedbackId}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {testResults.abTestAssignment && (
              <div className="border rounded-lg p-4">
                <h3 className="text-lg font-semibold mb-2">🎯 A/B测试分配测试</h3>
                <div className={`p-3 rounded ${testResults.abTestAssignment.success ? 'bg-green-50' : 'bg-red-50'}`}>
                  <p className={testResults.abTestAssignment.success ? 'text-green-700' : 'text-red-700'}>
                    {testResults.abTestAssignment.message}
                  </p>
                  {testResults.abTestAssignment.readiness && (
                    <div className="mt-2 text-sm text-gray-600">
                      <p>样本数: {testResults.abTestAssignment.readiness.sampleSize}</p>
                      <p>就绪状态: {testResults.abTestAssignment.readiness.isReady ? '是' : '否'}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {testResults.dataAnalysis && (
              <div className="border rounded-lg p-4">
                ...
                <div className={`p-3 rounded ${testResults.dataAnalysis.success ? 'bg-green-50' : 'bg-red-50'}`}>
                  <p className={testResults.dataAnalysis.success ? 'text-green-700' : 'text-red-700'}>
                    {testResults.dataAnalysis.message}
                  </p>
                  {testResults.dataAnalysis.dataQuality && (
                    <div className="mt-2 text-sm text-gray-600">
                      <p>总会话数: {testResults.dataAnalysis.dataQuality.totalSessions}</p>
                      <p>总反馈数: {testResults.dataAnalysis.dataQuality.totalFeedback}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            {testResults.apiEndpoints && (
              <div className="border rounded-lg p-4">
                ...
                <div className={`p-3 rounded ${testResults.apiEndpoints.success ? 'bg-green-50' : 'bg-red-50'}`}>
                  <p className={testResults.apiEndpoints.success ? 'text-green-700' : 'text-red-700'}>
                    {testResults.apiEndpoints.message}
                  </p>
                  {testResults.apiEndpoints.results && (
                    <div className="mt-2 text-sm text-gray-600">
                      <p>会话API: {testResults.apiEndpoints.results.sessionAPI?.success ? '✅' : '❌'}</p>
                      <p>反馈API: {testResults.apiEndpoints.results.feedbackAPI?.success ? '✅' : '❌'}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="border-2 border-blue-500 rounded-lg p-6 bg-blue-50">
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{testResults.finalReport.totalTests}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">成功率</p>
                    <p className="text-2xl font-bold text-green-600">{testResults.finalReport.successRate}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-600">通过测试</p>
                    <p className="text-xl font-semibold text-green-600">{testResults.finalReport.successfulTests}</p>
                  </div>
                  <div>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-1">
                    {Object.entries(testResults.finalReport.summary).map(([key, value]) => (
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {testResults.finalReport.recommendations.map((rec, index) => (
