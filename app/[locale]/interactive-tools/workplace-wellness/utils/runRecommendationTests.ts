...
import { generateRecommendations, generateColdStartRecommendations } from './recommendationEngine';
import { createTestPeriodData, createTestWorkImpactData, createTestNutritionData } from './recommendationTestUtils';
...
function validateRecommendationQuality() {
  ...
  if (avgScore >= 50) {
    ...
    console.log('   ⚠️  推荐分数: 偏低（可能需要调整评分算法）');
  }
}
...
function performanceTest() {
  ...
  if (avgTime < 200) {
    ...
    console.log('   ⚠️  性能: 可接受 (200-500ms)');
  } else {
...
function edgeCaseTests() {
  ...
  try {
    ...
    console.log(`✅ 空数据测试: ${emptyResult.recommendations.length} 个推荐`);
    if (emptyResult.recommendations.length > 0) {
      console.log('   ✓ 冷启动处理: 正常');
    } else {
      console.log('   ⚠️  冷启动处理: 未返回推荐');
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (urgentCount > 0) {
      ...
      console.log('   ⚠️  极端情况处理: 应该推荐高优先级内容');
    }
  } catch (error: any) {
  ...
  try {
    ...
    console.log(`✅ 冷启动测试: ${coldStartItems.length} 个通用推荐`);
    ...
    console.log(`❌ 冷启动测试: 错误 - ${error.message}`);
  }
}
