  ...
  const faqs = isZh ? [
    {
      ...
      answer: '如果疼痛等级达到7分以上（满分10分），或者疼痛严重影响日常生活和工作，建议就医评估。特别是出现突发剧烈疼痛、异常出血、发热等症状时，应立即就医。'
    },
    {
      question: '痛经的危险信号有哪些？',
      answer: '7个主要危险信号包括：突发剧烈疼痛、异常大量出血、发热和全身症状、疼痛模式突然改变、非经期疼痛、疼痛进行性加重、止痛药无效。出现任何一个都应该重视。'
    }
  ] : [
    {
      question: 'What level of period pain requires medical attention?',
      answer: 'If pain reaches 7 or above on a 10-point scale, or severely affects daily life and work, medical evaluation is recommended. Especially when experiencing sudden severe pain, abnormal bleeding, or fever, seek immediate medical care.'
    },
    {
...
export function generateHowToStructuredData(locale: string) {
  ...
  return {
    '@context': 'https://schema.org',
    '@type': 'HowTo',
    'name': isZh ? '如何评估痛经是否需要就医' : 'How to Assess if Period Pain Requires Medical Care',
    'description': isZh ? '通过3个步骤科学评估痛经症状，判断是否需要就医' : 'Scientifically assess period pain symptoms in 3 steps to determine if medical care is needed',
    'totalTime': 'PT10M',
    'estimatedCost': {
      '@type': 'MonetaryAmount',
      'currency': 'USD',
...
// 生成完整的元数据
export function generateEnhancedMetadata(locale: string, translations: any): Metadata {
  const isZh = locale === 'zh';
